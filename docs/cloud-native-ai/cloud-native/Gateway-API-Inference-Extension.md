...
#### 1.4.3 请求进来时：一步一步解析、经过哪些服务、最终到达模型
...
### 4.2 Predicted Latency Scorer（延迟预测，核心“智能”）
...
#### 4.2.5 Composite 分数（无预测或 composite-only 时）
...
### 4.3 Queue / KV Cache Utilization / Running Requests Size Scorer
...
## 五、Weighted Random Picker（从分数到概率）
...
### 7.1 Predicted Latency Scorer（若启用）
