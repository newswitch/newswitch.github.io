...
默认以容器化方式部署驱动；宿主机已装驱动时须设 `driver.enabled=false`。预装驱动**不受** Operator 生命周期管理。Helm 安装见：[使用 Helm 部署 GPU Operator](./10-使用%20Helm%20部署%20GPU%20Operator.md)。
...
升级时需停止 GPU 客户端、卸载旧模块、加载新模块并重新启用客户端——**不同于**普通无状态 DaemonSet 滚动更新。细节见：[升级、回滚与节点维护](./12-GPU%20Operator%20升级、回滚与节点维护.md)。
...
下一篇：[GPU Operator 升级、回滚与节点维护](./12-GPU%20Operator%20升级、回滚与节点维护.md)。
