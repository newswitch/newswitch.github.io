...
Kubernetes 通过 [Device Plugin](./05-Kubernetes%20如何识别和管理%20GPU.md)，让 Pod 访问 GPU 这类特殊硬件。
...
- 手动：[驱动 / Toolkit](./04-NVIDIA%20驱动、CUDA%20与容器运行时的关系.md) + Device Plugin
- 自动化：[GPU Operator](./09-NVIDIA%20GPU%20Operator%20架构与组件说明.md)
- 分配链路：[Pod 如何使用上 GPU](./06-Pod如何使用上GPU：Device%20Plugin与Container%20Toolkit.md)
...
除通用 NFD 外，常还需要能打出「业务可用」标签的插件（通用或厂商特定）。NVIDIA 侧可参考 GPU Operator 自带的 NFD / GFD 能力，详见 [GPU Operator 一文](./09-NVIDIA%20GPU%20Operator%20架构与组件说明.md)。
...
4. **GPU 节点建议配合 Taint**，防止普通业务误调度（见后续 [Taint 与 Toleration 实践](./14-GPU%20节点%20Taint%20与%20Toleration%20实践.md)）  
5. 需要细粒度共享时，再看 [Time-Slicing](./20-Kubernetes%20GPU%20Time-Slicing%20配置实践.md) / [HAMi](./22-HAMi%20vGPU%20原理与实践.md) / MIG，而不是只改 `nvidia.com/gpu: 1` 的语义
