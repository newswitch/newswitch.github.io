...
只看 GPU 利用率，不知道用户是否卡顿；只看 TTFT，不知道该扩卡还是修网络。把 **vLLM（或同类引擎）业务指标** 与 **DCGM GPU 指标** 对齐到同一时间轴，才能做容量与排障。前置：[第 28](./28-大模型推理服务性能指标设计.md)、[第 38](./38-DCGM%20Exporter%20GPU%20监控指标详解.md)、[第 40](./40-Grafana%20GPU%20集群总览看板设计.md) 篇。
