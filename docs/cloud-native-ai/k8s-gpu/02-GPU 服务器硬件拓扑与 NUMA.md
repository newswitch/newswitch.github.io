...
命令细节也可对照：[nvidia-smi 常用命令与指标说明](./03-nvidia-smi%20常用命令与指标说明.md)。
...
后续：[InfiniBand、RoCE 与 GPU 集群网络](./34-InfiniBand、RoCE%20与%20GPU%20集群网络.md)、[GPU 集群拓扑感知调度](./35-GPU%20集群拓扑感知调度.md)。
