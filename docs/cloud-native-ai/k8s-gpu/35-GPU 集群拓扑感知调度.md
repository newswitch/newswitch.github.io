...
同样 8 张卡，**同 NVLink 域内** 的 AllReduce 与 **跨 PCIe / 跨 NUMA / 跨交换机** 可以差一个数量级。拓扑感知调度的目标：让需要密通信的进程，尽量落在「高速互连」的 GPU 集合上。本文结合 `nvidia-smi topo`、NCCL [GPU troubleshooting](https://docs.nvidia.com/deeplearning/nccl/user-guide/docs/troubleshooting/gpu_troubleshooting.html) 与 Kubernetes 调度实践。前置：[第 33](./33-NCCL%20通信原理与常见问题.md)、[第 02 篇骨架](./02-GPU%20服务器硬件拓扑与%20NUMA.md)（硬件细节可后补）。
...
- [GFD](./05b-NVIDIA-Device-Plugin部署与配置.md) / Node Feature Discovery：暴露型号、驱动、部分 PCIe 信息  
...
网络侧基线见 [34](./34-InfiniBand、RoCE%20与%20GPU%20集群网络.md)；超时复盘见 [48](./48-NCCL%20Timeout%20排查流程.md)。
