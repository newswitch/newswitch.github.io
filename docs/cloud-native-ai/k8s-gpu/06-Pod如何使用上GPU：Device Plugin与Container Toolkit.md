...
- [驱动 / CUDA / 容器运行时](./04-NVIDIA%20驱动、CUDA%20与容器运行时的关系.md)：环境怎么搭起来
- [Device Plugin 机制](./05-Kubernetes%20如何识别和管理%20GPU.md)：扩展资源怎么接入
- [GPU Operator](./09-NVIDIA%20GPU%20Operator%20架构与组件说明.md)：生产里怎么自动化安装
...
下一篇可继续写生产向的 Pod 配置与 Pending 排查：[Kubernetes GPU Pod 配置详解](./07-Kubernetes%20GPU%20Pod%20配置详解.md)、[GPU Pod 一直 Pending 的排查流程](./08-GPU%20Pod%20一直%20Pending%20的排查流程.md)。
