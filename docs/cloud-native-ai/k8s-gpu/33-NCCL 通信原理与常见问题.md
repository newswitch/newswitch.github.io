...
本文讲清：NCCL 在干什么、**Ring / Tree**、日志与关键环境变量。网络专篇见 [34](./34-InfiniBand、RoCE%20与%20GPU%20集群网络.md)，拓扑与调度见 [35](./35-GPU%20集群拓扑感知调度.md)，超时复盘见 [48](./48-NCCL%20Timeout%20排查流程.md)。
...
健康 8 卡 PCIe 全互连时，`-p2p p` 矩阵应大量为 `OK`（对角为 `X`）。官方说明：即便矩阵显示 OK，NCCL 仍异常时，常见原因是 **PCI ACS** 干扰 GPU Direct——见 [GPU troubleshooting](https://docs.nvidia.com/deeplearning/nccl/user-guide/docs/troubleshooting/gpu_troubleshooting.html) 与 [第 35 篇](./35-GPU%20集群拓扑感知调度.md)。
...
网卡命名与 IB/RoCE 细节见 [第 34 篇](./34-InfiniBand、RoCE%20与%20GPU%20集群网络.md)。完整列表见官方 [Environment Variables](https://docs.nvidia.com/deeplearning/nccl/user-guide/docs/env.html)。
