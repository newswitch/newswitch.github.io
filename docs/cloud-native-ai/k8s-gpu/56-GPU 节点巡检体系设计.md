...
> 节点名、路径、指标阈值请换成真实环境。巡检 ≠ 只跑 `nvidia-smi`。深度 DCGM 诊断会占 GPU，**勿在承载业务的卡上直接跑**。监控指标见 [第 38 篇](./38-DCGM%20Exporter%20GPU%20监控指标详解.md)。
...
下一篇：[生产级架构设计](./57-生产级%20Kubernetes%20GPU%20集群架构设计.md)。
