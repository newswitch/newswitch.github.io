...
值班常见矛盾现象：**`FB_USED` 很高，`GPU_UTIL` / `GR_ENGINE_ACTIVE` 却长期很低**。卡被「占着」但不干活，既浪费钱又拖死调度。本文给出用 DCGM + 业务指标的分析路径。前置：[第 38](./38-DCGM%20Exporter%20GPU%20监控指标详解.md)、[第 40](./40-Grafana%20GPU%20集群总览看板设计.md) 篇。
...
- 证据：业务指标 + FB_USED  
- 处理：降并发、扩副本、查前缀缓存；见 [28](./28-大模型推理服务性能指标设计.md)、[42](./42-大模型业务指标与%20GPU%20指标关联分析.md)  
...
- 处理：[48](./48-NCCL%20Timeout%20排查流程.md)  
...
告警模板见 [第 39 篇](./39-Prometheus%20GPU%20告警策略设计.md) 的 `GPUIdleButAllocated`。
...
下一篇：[业务指标与 GPU 指标关联分析](./42-大模型业务指标与%20GPU%20指标关联分析.md)。
