...
组件原理见：[NVIDIA GPU Operator 架构与组件说明](./09-NVIDIA%20GPU%20Operator%20架构与组件说明.md)。驱动预装 vs Operator 安装见：[两种驱动管理模式](./11-GPU%20Operator%20两种驱动管理模式.md)。
...
预期日志可见型号、驱动、显存、CUDA 兼容版本。Pending 排查：[GPU Pod 一直 Pending](./08-GPU%20Pod%20一直%20Pending%20的排查流程.md)。
...
下一篇：[GPU Operator 两种驱动管理模式](./11-GPU%20Operator%20两种驱动管理模式.md)。
