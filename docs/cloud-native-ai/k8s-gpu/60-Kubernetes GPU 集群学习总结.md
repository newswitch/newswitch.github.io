...
> 实验记录、版本与验收结果请填入自己的真实数据。总路线见 [00](./00-Kubernetes-GPU集群学习路线.md)；进阶 DRA 见 [61](./61-Kubernetes%20DRA%20概念与核心%20API（v1.35+）.md)、[62](./62-DRA%20集群安装与设备分配实践（v1.34+）.md)。
...
本总结对应本专栏 [学习路线](./00-Kubernetes-GPU集群学习路线.md) 各阶段正文与官方 NVIDIA / Kubernetes / vLLM / Volcano 文档实践路径。
