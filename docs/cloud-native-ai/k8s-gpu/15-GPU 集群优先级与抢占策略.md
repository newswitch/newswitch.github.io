...
GPU 不够时，谁先跑、谁可以挤掉谁，决定 SLA。Kubernetes 用 **PriorityClass** 做 Pod 优先级与抢占；批处理队列（Volcano / Kueue）还有各自的抢占语义。本文说明原生层怎么配，以及和队列层怎么分工。多租户总览见 [第 52 篇](./52-GPU%20多租户与资源配额设计.md)；Volcano 见 [16～18](./16-Volcano%20GPU%20调度器入门.md)。
...
- **节点池污点**：推理池只收高优先级 + 对应 toleration（[第 51 篇](./51-生产%20GPU%20集群节点池规划.md)）  
- **ResourceQuota**：限制低优先级租户的 GPU 上限（[第 52 篇](./52-GPU%20多租户与资源配额设计.md)）  
...
- 训练被抢应能从 **Checkpoint** 恢复（[第 32 篇](./32-训练任务%20Checkpoint%20与断点恢复.md)）  
