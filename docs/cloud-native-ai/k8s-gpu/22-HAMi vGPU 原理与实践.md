...
本文介绍开源 GPU 虚拟化方案 **HAMi**：安装、配置与使用。相对 [Time-Slicing](./20-Kubernetes%20GPU%20Time-Slicing%20配置实践.md) 这类「只共享、不隔离」的方案，HAMi 还能对 **GPU core / memory** 做限制，让共享同一张卡的多个 Pod 更容易拿到稳定配额。
...
- [驱动 / CUDA / 容器运行时](./04-NVIDIA%20驱动、CUDA%20与容器运行时的关系.md)
- [GPU Operator](./09-NVIDIA%20GPU%20Operator%20架构与组件说明.md)
- [Device Plugin 机制](./05-Kubernetes%20如何识别和管理%20GPU.md)
- [Pod 如何使用上 GPU](./06-Pod如何使用上GPU：Device%20Plugin与Container%20Toolkit.md)
...
HAMi 依赖 NVIDIA 那一套（驱动、Toolkit、基础 Device Plugin 生态等），建议先装 [GPU Operator](./09-NVIDIA%20GPU%20Operator%20架构与组件说明.md)，再装 HAMi。
...
选型时可与 [整卡 / Time-Slicing / MPS / MIG 对比](./19-GPU%20整卡独占、Time-Slicing、MPS%20与%20MIG%20对比.md) 一起看：需要**软件级细配额、且卡型未必支持 MIG** 时，HAMi 往往更合适；要**硬件级隔离**则优先 MIG（A100/H100 等）。

装完后建议继续做隔离实测：[HAMi Core 与 Memory 隔离测试](./22b-HAMi-Core与Memory隔离测试.md)。
