...
单卡训练脚本能跑通后，上集群会立刻碰到：多进程怎么编号、谁当 Master、何时做集合通信、为什么「起了一半 Worker」会占死 GPU。本文给第六阶段搭地图——并行范式、进程角色、Rendezvous、以及在 Kubernetes / Volcano 上的任务生命周期。代码级 DDP 见 [第 30 篇](./30-PyTorch%20DDP%20在%20Kubernetes%20中的部署.md)；Gang 见 [第 18 篇](./18-Gang%20Scheduling%20在分布式训练中的作用.md)。
...
本阶段以 **DDP** 为主线；显存不够再看 [第 31 篇 ZeRO](./31-DeepSpeed%20ZeRO%20与%20GPU%20显存优化.md)；通信细节见 [第 33 篇 NCCL](./33-NCCL%20通信原理与常见问题.md)。
...
继续：[PyTorch DDP 在 Kubernetes 中的部署](./30-PyTorch%20DDP%20在%20Kubernetes%20中的部署.md)。
