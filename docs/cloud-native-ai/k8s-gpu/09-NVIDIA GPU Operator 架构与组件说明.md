...
上一篇 [NVIDIA 驱动、CUDA 与容器运行时的关系](./04-NVIDIA%20驱动、CUDA%20与容器运行时的关系.md) 讲了裸机 / Docker / Kubernetes 手动用 GPU 的路径。流程不复杂，但节点上要装驱动、Container Toolkit、Device Plugin 等，集群一大就很麻烦。
