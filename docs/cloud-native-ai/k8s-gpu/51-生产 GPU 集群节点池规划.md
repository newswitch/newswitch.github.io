...
生产集群很少「所有 GPU 节点一个池」。按 **型号、用途、网络拓扑** 切开，才能控 SLA、成本和嘈杂邻居。本文给出节点池模型，并用 [Kueue ResourceFlavor](https://kueue.sigs.k8s.io/zh-cn/docs/concepts/resource_flavor/) / [拓扑感知调度](https://kueue.sigs.k8s.io/zh-cn/docs/concepts/topology_aware_scheduling/) 表达「哪种卡、哪一层拓扑」。前置：[污点](./14-GPU%20节点%20Taint%20与%20Toleration%20实践.md)（若仍为骨架，先理解污点概念）、[拓扑](./35-GPU%20集群拓扑感知调度.md)。
...
与本系列 [第 35 篇](./35-GPU%20集群拓扑感知调度.md) 的 `nvidia-smi topo` 互补：一个管 **机内 NVLink**，一个管 **机架/区块** 放置。
...
下一篇：[多租户与资源配额](./52-GPU%20多租户与资源配额设计.md)。
