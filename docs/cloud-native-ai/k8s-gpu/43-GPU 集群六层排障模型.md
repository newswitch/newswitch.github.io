...
`lspci` 有卡、`nvidia-smi` 失败 → 驱动/NVML；`lspci` 无卡 → 硬件优先；`nvidia-smi` 正常 → 宿主机链路大体 OK。退出码可辅助（如 9 驱动未加载、12 NVML 库、15 掉总线）——见 [第 44 篇](./44-nvidia-smi%20失败排查.md)。
...
物理 8 卡、Allocatable 7 → 查 unhealthy / Xid。详见 [第 47 篇](./47-NVIDIA%20Xid%20错误排查.md)。
...
Pending 系统排查见 [第 08 篇](./08-GPU%20Pod%20一直%20Pending%20的排查流程.md)。

### 第六层：CUDA 应用

容器已有设备后仍失败：`nvidia-smi` vs `torch.cuda.is_available()`、镜像是否 CUDA 版、兼容性、OOM、NCCL、TP 数量等。见 [第 45](./45-Pod%20分配%20GPU%20后看不到%20GPU.md)、[46](./46-CUDA%20OOM%20排查与优化.md)、[48](./48-NCCL%20Timeout%20排查流程.md)。
...
下一篇：[nvidia-smi 失败排查](./44-nvidia-smi%20失败排查.md)。
