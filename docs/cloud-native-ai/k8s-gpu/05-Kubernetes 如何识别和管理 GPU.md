...
AI 负载还需要 GPU。在 [NVIDIA 驱动、CUDA 与容器运行时的关系](./04-NVIDIA%20驱动、CUDA%20与容器运行时的关系.md) 里可以看到：Kubernetes 侧靠 **Device Plugin** 把节点上的 GPU 暴露成扩展资源，使用方式接近原生资源。
...
![Device Plugin Framework Overview](/images/k8s-gpu/05-Device-Plugin/deviceplugin-framework-overview.svg)
...
结合 [GPU Operator](./09-NVIDIA%20GPU%20Operator%20架构与组件说明.md) 安装的 **NVIDIA Container Toolkit**：启动容器时识别该环境变量，再把对应 GPU 设备挂进容器。
...
NVIDIA 官方插件的安装、Helm、Time-Slicing / MPS、GFD 等见：[NVIDIA Device Plugin 部署与配置](./05b-NVIDIA-Device-Plugin部署与配置.md)。

下一篇把「申请 GPU → 容器内可用」整条链路串起来：[Pod 如何使用上 GPU：Device Plugin 与 Container Toolkit](./06-Pod如何使用上GPU：Device%20Plugin与Container%20Toolkit.md)。Pending 排查见：[GPU Pod 一直 Pending 的排查流程](./08-GPU%20Pod%20一直%20Pending%20的排查流程.md)。
