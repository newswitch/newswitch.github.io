...
容量规划回答三件事：**现在够不够、何时买卡、买什么型号/放哪一池**。输入来自 DCGM 与队列积压；输出是节点池与 `nominalQuota` 调整。前置：[监控 38～42](./38-DCGM%20Exporter%20GPU%20监控指标详解.md)、[多租户 52](./52-GPU%20多租户与资源配额设计.md)、[节点池 51](./51-生产%20GPU%20集群节点池规划.md)。
...
- 「高显存低利用率」时长（[第 41 篇](./41-GPU%20利用率低但显存占满怎么分析.md)）  
- 推理：KV 使用率与 TTFT（[第 42 篇](./42-大模型业务指标与%20GPU%20指标关联分析.md)）  
...
容量 = **测准单卡能力** × **业务峰值** × **缓冲**，再用监控验证「缺的是卡、是配额还是拓扑」。下一篇把钱算清楚：[成本与利用率](./54-GPU%20集群成本与利用率分析.md)。
