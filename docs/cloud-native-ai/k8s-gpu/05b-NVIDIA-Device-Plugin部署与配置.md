...
本文是 [NVIDIA/k8s-device-plugin](https://github.com/NVIDIA/k8s-device-plugin) 官方 README 的中文整理，说明如何在 Kubernetes 中部署、配置 NVIDIA Device Plugin，以及 Time-Slicing / MPS、Helm、GFD 等能力。原理见 [Device Plugin 机制](./05-Kubernetes%20如何识别和管理%20GPU.md)，分配链路见 [Pod 如何使用上 GPU](./06-Pod如何使用上GPU：Device%20Plugin与Container%20Toolkit.md)。
...
驱动与 Toolkit 安装可参考官方：[Container Toolkit Install Guide](https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/install-guide.html)，以及本系列 [驱动 / CUDA / 容器](./04-NVIDIA%20驱动、CUDA%20与容器运行时的关系.md)。
...
**警告：** 若使用了 Device Plugin，但容器**没有申请** GPU，插件可能把机器上**所有 GPU** 暴露进该容器。务必在 `limits` 中显式申请（见 [GPU Pod 配置](./07-Kubernetes%20GPU%20Pod%20配置详解.md)）。
...
更细的实践可另见本系列 [Time-Slicing 配置实践](./20-Kubernetes%20GPU%20Time-Slicing%20配置实践.md)、[整卡/共享/MIG 对比](./19-GPU%20整卡独占、Time-Slicing、MPS%20与%20MIG%20对比.md)。
