...
有 DCGM 指标之后，要把「异常」变成可行动的告警：太敏感会疲劳，太迟钝会漏掉掉卡。本文给出分级思路与示例规则，指标来源见 [第 38 篇](./38-DCGM%20Exporter%20GPU%20监控指标详解.md)；栈部署见 [kube-prometheus](https://docs.nvidia.com/datacenter/cloud-native/gpu-telemetry/latest/kube-prometheus.html)。
...
与 NCCL 跨机问题交叉时，再查 [34](./34-InfiniBand、RoCE%20与%20GPU%20集群网络.md) / [48](./48-NCCL%20Timeout%20排查流程.md)。
...
若已抓 vLLM `/metrics`（[第 28 篇](./28-大模型推理服务性能指标设计.md)）：
...
下一篇：[Grafana GPU 集群总览看板设计](./40-Grafana%20GPU%20集群总览看板设计.md)。
