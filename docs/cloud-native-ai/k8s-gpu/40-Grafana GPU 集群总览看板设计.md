...
单卡下钻时与 `nvidia-smi` / `topo -m` 对照（[第 35 篇](./35-GPU%20集群拓扑感知调度.md)）。
...
数据源仍是 Prometheus，但指标来自 **vLLM `/metrics`**（[第 28 篇](./28-大模型推理服务性能指标设计.md)）与 DCGM **同屏**：
...
关联方法展开见 [第 42 篇](./42-大模型业务指标与%20GPU%20指标关联分析.md)。
...
下一篇：[GPU 利用率低但显存占满怎么分析](./41-GPU%20利用率低但显存占满怎么分析.md)。
