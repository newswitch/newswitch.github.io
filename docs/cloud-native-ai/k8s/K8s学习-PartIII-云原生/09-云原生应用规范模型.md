...
:::info 转载说明
本文整理自 [Jimmy Song《Kubernetes 教程》](https://jimmysong.io/zh/book/kubernetes-handbook/) 对应章节，原文采用 [CC BY-NC-SA 4.0](https://creativecommons.org/licenses/by-nc-sa/4.0/deed.zh) 协议。原作者：Jimmy Song；原页：[云原生应用规范模型](https://jimmysong.io/zh/book/kubernetes-handbook/cloud-native/spec/)。仅供个人非商业学习；若有勘误请以上游为准。
:::
...
### 应用运维人员（Application Operator）

- 创建 `ApplicationConfiguration`，组合 `Component` 和 `Trait`
- 配置运维特性如扩缩容、路由、监控等
- 管理应用的部署、升级和监控

### 基础设施运维人员（Infrastructure Operator）
