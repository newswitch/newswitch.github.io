...
sidebar_label: "01. Kubernetes 网络架构概述"
sidebar_position: 1
...
![容器网络挑战与解决方案](/images/k8s/networking/overview/0eff034308869028feb90e389fbb9dc9.svg)
...
![Kubernetes 网络模型层次](/images/k8s/networking/overview/a4bbd72e29663ed17784d3a460b6be8e.svg)

### 2.1 集群网络 (Cluster Network) {/* #集群网络-cluster-network */}

- 整个 Kubernetes 集群的网络范围，通常为一个大的 CIDR 块（如 `10.0.0.0/8`）。
- 由网络插件负责分配和管理。

### 2.2 节点网络 (Node Network) {/* #节点网络-node-network */}

- 每个节点的网络配置，包括节点 IP 和路由规则。
- 负责节点间通信。

### 2.3 Pod 网络 (Pod Network) {/* #pod-网络-pod-network */}

- Pod 的网络命名空间，每个 Pod 有唯一的 IP 地址。
- 支持跨节点 Pod 间通信。

## 3. 通信模式与实现机制 {/* #通信模式与实现机制 */}
...
![Kubernetes 通信模式](/images/k8s/networking/overview/ed10c47c51e23dabd2fdb5b3270b6c83.svg)

### 3.1 Pod-to-Pod 通信 {/* #pod-to-pod-通信 */}

- 所有 Pod 在同一个扁平网络中，无需 NAT 转换，可直接通信。
- 由 CNI 插件实现。

### 3.2 Pod-to-Service 通信 {/* #pod-to-service-通信 */}

- 通过 Service 抽象层，提供负载均衡和服务发现。
- 支持多种 Service 类型。

## 4. Service 网络抽象与类型 {/* #service-网络抽象与类型 */}
...
![Service 类型和网络实现](/images/k8s/networking/overview/4ebaa33bd556d934614ecf1f2bde3e14.svg)
...
![Service Endpoint 和 Network Topology](/images/k8s/networking/overview/9705a5817a3df36df5cd729d51fe61f0.svg)

## 5. 网络策略与安全机制 {/* #网络策略与安全机制 */}
...
![NetworkPolicy 模型结构](/images/k8s/networking/overview/b27357cdc1528705517adf02c94dd6ac.svg)
...
![CNI Plugin Classification](/images/k8s/networking/overview/b9e21b4ea0fe56a3ac4eae0fc26202e2.svg)
...
![网络架构演进](/images/k8s/networking/overview/9481cfad0c68a2719ca21c4476b0c6a4.svg)
...
![网络故障排查流程](/images/k8s/networking/overview/b8af066a8173f5f5efb2f5c09ad05153.svg)
