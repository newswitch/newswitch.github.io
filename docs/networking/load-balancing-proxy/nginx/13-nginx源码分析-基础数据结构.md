...
sidebar_position: 13
description: "Nginx 源码不适合从 main() 第一行一路读到结尾。它由配置生命周期、Master/Worker 进程模型、 事件循环、模块系统、连接/请求对象和一组生命周期明确的数据结构共同组成。"
...
实现见[深入篇：基础数据结构（一）](./14-nginx源码解析-基础数据结构（一）.md)，两篇文章不重复展开
同一段代码。
...
### 11.1 实验一：观察 Request Pool {/* #实验一观察-request-pool */}
...
### 11.2 实验二：观察 Timer Rbtree {/* #实验二观察-timer-rbtree */}
...
### 11.3 实验三：追踪一条 Proxy 请求 {/* #实验三追踪一条-proxy-请求 */}
...
- [深入篇：内存池与基础数据结构实现](./14-nginx源码解析-基础数据结构（一）.md)
