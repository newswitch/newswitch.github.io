...
Clos 不是某个协议，而是一类多级交换拓扑。Spine-Leaf 常被称为两层 Folded Clos（折叠 Clos）；从跨机架转发路径看，是源 Leaf、Spine、目标 Leaf 三段交换。
...
5. N-1 设计负载／上联容量：`800/600 = 1.33`；按标称端口计算的物理超售比则是 `1600/600 ≈ 2.67:1`。

因此，该方案满足“故障后设计负载不超过剩余容量两倍”，但**不满足标称端口 N-1 超售不超过 2:1**。若要满足后者，需要剩余上联至少 800G；例如 4 台 Spine、每台 3 条 100G 上联，失去一台后剩 900G，标称超售约为 1.78:1。还需确认 Leaf 端口与对端容量允许这种连接。
...
一张网络的输入应包含流量矩阵 `T(i,j)`：源机架 i 发往目的机架 j 的需求。各机架出口求和可检查 Leaf 上联，目的机架入口求和可检查接收侧，跨一组机架边界的流量则要与相应网络截面的容量比较。

例如 4 个机架各有 200G 上联，总上联 800G；若其中 3 个机架同时各向第 4 个发送 100G，目标侧仅有 200G 接入能力，至少存在 100G 的持续超额输入。全网总发送量只占 800G 的 37.5%，也不能防止这个热点。

拓扑上可路由的容量、给定流量矩阵能否分配、ECMP 实际能否分散，是三种不同问题。严格无阻塞、可重排无阻塞等交换网络概念有各自假设，不能用一张“上下行 1:1”端口表直接证明。

### 12.2 单流和多流怎样使用平行路径

普通按流 ECMP 让同一流稳定走一个成员以减少乱序。增加 Spine 可以增加并发流的总承载，但一条流通常仍受单路径端口速率限制。只有多流且散列充分分散时，总带宽才可能接近聚合容量。

假设有四条等价 100G 路径，两条 80G 大流碰巧落在同一条上，即使其余路径空闲，该成员也会拥塞。流量较多并不代表统计一定均匀；大象流数量、散列字段、隧道熵和成员变动后的重映射都影响结果。设计背景见 [RFC 7938](https://www.rfc-editor.org/rfc/rfc7938.html)。

### 12.3 双网卡容量应该记在哪一侧

每台服务器双 25G，不代表任意时候都可发送 50G：Active/Standby Bond 正常只用一条，LACP 单流通常只用一个成员，双归属失去一台 Leaf 后也会损失接入容量。

容量表应分别注明服务器逻辑聚合、每台 Leaf 实际连接的物理端口、正常可用量和故障可用量。若一台服务器的两根线接到两台 Leaf，不应把两根线都重复记入每台 Leaf 下行总和。

### 12.4 思考与解答

**800G 设计负载除以 600G，能证明物理超售低于 2:1 吗？**

不能。它是负载与容量比；标称下行 1600G 时，物理超售是约 2.67:1。

**全网平均 40% 利用率，能排除拥塞吗？**

不能。特定出口、某条 ECMP 成员和短时间队列都可能超载，平均值掩盖了空间与时间分布。

**两台 Spine 增加为四台，单个 TCP 流吞吐会翻倍吗？**

普通按流 ECMP 下不会因此保证翻倍。需检查单路径速率、散列、窗口与端点能力。

**服务器双 25G，容量表应该固定写 50G 吗？**

应分模式与状态。Active/Standby、LACP、多路径应用及单 Leaf 故障后的实际可用量不同。
