...
sidebar_label: "02. RDMA技术详解（二）：RDMA Send Receive操作"
sidebar_position: 2
description: "RDMA指的是远程直接内存访问，这是一种通过网络在两个应用程序之间搬运缓冲区里的数据的方法。RDMA与传统的网络接口不同，因为它绕过了操作系统。这允许实现了RDMA的程序具有如下特点："
tags: [RDMA, 网络, 高性能计算]
...
建立数据通道的API是一种称之为"verbs"的API。"verbs" API是由一个叫做OFED的Linux开源项目维护的。在站点http://www.openfabrics.org上。

## 3. Queue Pairs {/* #queue-pairs */}
...
RDMA Memory Registration（RDMA内存注册）
```c
struct ibv_mr {
...
## 4. RDMA Send/Receive {/* #rdma-sendreceive */}
...
![RDMA-5.png](/images/RDMA技术详解（二）/RDMA-5.png)
...
![RDMA-6.png](/images/RDMA技术详解（二）/RDMA-6.png)

我们刚刚举例说明的是一个RDMA Send操作。在IB或RoCE中，传送一个小缓冲区里的数据耗费的总时间大约在1.3µs。通过同时创建很多WQE, 就能在1秒内传输存放在数百万个缓冲区里的数据。

## 5. 总结 {/* #总结 */}
本章介绍了 RDMA Verbs API、工作队列及 Send/Receive 操作，说明缓冲区数据如何通过 Queue Pair 从一台主机传输到另一台主机。理解队列、Work Request 和 Completion Queue 的关系，是继续学习 RDMA Read/Write 和性能调优的基础。
