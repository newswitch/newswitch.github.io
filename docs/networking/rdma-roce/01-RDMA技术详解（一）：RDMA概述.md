...
sidebar_label: "01. RDMA技术详解（一）：RDMA概述"
sidebar_position: 1
description: "DMA(直接内存访问)是一种能力，允许在计算机主板上的设备直接把数据发送到内存中去，数据搬运不需要CPU的参与。"
tags: [RDMA, 网络, 高性能计算]
...
## 4. RDMA 三种不同的硬件实现 {/* #rdma-三种不同的硬件实现 */}
...
## 6. 核心概念 {/* #核心概念 */}
### 6.1 Memory Registration(MR) | 内存注册 {/* #memory-registrationmr--内存注册 */}
...
## 7. RDMA数据传输 {/* #rdma数据传输 */}
### 7.1 RDMA Send | RDMA发送(/接收)操作 （Send/Recv） {/* #rdma-send--rdma发送接收操作-sendrecv */}
...
### 7.2 RDMA Read | RDMA读操作 (Pull) {/* #rdma-read--rdma读操作-pull */}
RDMA读操作本质上就是Pull操作, 把远程系统内存里的数据拉回到本地系统的内存里。

### 7.3 RDMA Write | RDMA写操作 (Push) {/* #rdma-write--rdma写操作-push */}
RDMA写操作本质上就是Push操作，把本地系统内存里的数据推送到远程系统的内存里。

### 7.4 RDMA Write with Immediate Data | 支持立即数的RDMA写操作 {/* #rdma-write-with-immediate-data--支持立即数的rdma写操作 */}
