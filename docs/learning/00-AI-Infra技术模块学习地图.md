...
AI 基础设施由 Linux、加速器、网络、存储、集群调度、训练推理、可观测性、性能工程和自动化等技术共同
构成。每个模块都有独立的原理和工程方法，也会在真实系统的数据路径与控制路径中相互连接。

本地图先介绍各模块的核心知识，再通过综合实战展示它们如何协同：
...
## 11. 技术学习优先级 {/* #10-技术学习优先级 */}

面向 AI 基础设施的建设与运维，可以按下面顺序学习：
...
模块优先级只决定学习先后，不改变技术本身的分类。例如错误预算属于“可观测性与可靠性”，不会因具体
应用场景改变技术归属。
...
做到这四点，才算把“了解某项技术”转化为“能够部署、分析、使用和排障”。
