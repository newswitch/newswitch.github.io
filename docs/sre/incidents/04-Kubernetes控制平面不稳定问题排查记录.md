...
sidebar_label: "04. Kubernetes 控制平面不稳定问题排查记录"
sidebar_position: 4
description: "本文完整记录了一次发生在 30+ 节点 Kubernetes 高可用集群中的典型控制平面不稳定问题。集群表现为「时好时坏」：kubectl 有时能正常执行，有时则报 connection refused。通过系统性排查，最终定位到根本原因是 etcd 节点磁盘 I/O 性能严重不足。文章详细还……"
...
## 2. 问题现象：难以捉摸的「时好时坏」 {/* #1-问题现象难以捉摸的时好时坏 */}
...
## 3. 初步定位：API Server 与 etcd 的关系 {/* #2-初步定位api-server-与-etcd-的关系 */}

### 3.1 检查 API Server 状态 {/* #21-检查-api-server-状态 */}
...
## 4. 关键突破：etcd 集群健康检查 {/* #3-关键突破etcd-集群健康检查 */}

### 4.1 确认 etcd 证书路径 {/* #31-确认-etcd-证书路径 */}
...
### 4.3 获取 etcd 集群详细状态 {/* #33-获取-etcd-集群详细状态 */}
...
## 5. 深入分析：定位 etcd 性能瓶颈 {/* #4-深入分析定位-etcd-性能瓶颈 */}

### 5.1 分析 etcd 日志 {/* #41-分析-etcd-日志 */}
...
## 6. 解决方案：迁移与优化 {/* #5-解决方案迁移与优化 */}

### 6.1 迁移 etcd 数据到专用 SSD {/* #51-迁移-etcd-数据到专用-ssd */}
...
### 6.2 优化 etcd 配置参数 {/* #52-优化-etcd-配置参数 */}
...
## 7. 生产环境加固建议 {/* #6-生产环境加固建议 */}

### 7.1 etcd 监控告警（Prometheus） {/* #61-etcd-监控告警prometheus */}
...
## 8. 排查流程：控制平面不稳定快速定位清单 {/* #7-排查流程控制平面不稳定快速定位清单 */}
