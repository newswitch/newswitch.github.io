...
sidebar_position: 1
description: "很多人把大模型（LLM）当作黑盒：输入一段话，输出一段话。但在工程与算法视角下，一个 Prompt 从「文本输入」到「逐 token 输出」会经历一条清晰的流水线：请求封装、分词、服务侧调度、Prefill、Decode、反解码。本文按链路拆解每一步发生了什么，以及它们如何对应到 TTFT/T……"
...
## 3. 从文本到 token：Tokenizer 做了什么 {/* #2-从文本到-tokentokenizer-做了什么 */}
...
## 4. 服务侧调度：为什么不是“直接跑模型” {/* #3-服务侧调度为什么不是直接跑模型 */}
...
## 7. 指标对应：TTFT 与 TPOT 为什么不同 {/* #6-指标对应ttft-与-tpot-为什么不同 */}
