...
sidebar_label: "91. vLLM 学习笔记（一）：整体代码架构"
sidebar_position: 91
description: "本文基于 vLLM 0.6.3 的 V0 架构，用于理解早期 LLMEngine、三状态队列和 GPU/CPU KV Swap。学习当前实现请先阅读 V1 整体架构与组件职责， 不要把本文类名和执行流程直接套用到 V1。"
...
## 1. vLLM 代码结构介绍 {/* #1-vllm-代码结构介绍 */}

![vLLM 整体架构](/images/vllm学习笔记（一）/vllm1.png)
...
### 1.1 Centralized Controller 集中控制器
...
### 1.2 Distributed Workers 分布式工作节点
...
## 2. vLLM 处理请求的流程 {/* #2-vllm-处理请求的流程 */}

当一个请求到达时，vLLM 的各个模块是如何协同工作的？下面按流程说明。

### 2.1 初始化并加载模型权重

![模型加载流程](/images/vllm学习笔记（一）/vllm2.png)
...
![KV Cache 块数量估计](/images/vllm学习笔记（一）/vllm3.png)
...
![KV Cache 预分配](/images/vllm学习笔记（一）/vllm4.png)

### 2.2 处理 Request
在完成了模型的加载和 KV cache 的预分配之后，vLLM 就可以开始处理用户的请求了。

#### 2.2.1 请求到达 LLMEngine
...
![请求到达与 Tokenization](/images/vllm学习笔记（一）/vllm5.png)
...
![调度器队列](/images/vllm学习笔记（一）/vllm6.png)
...
![执行推理与 CacheEngine](/images/vllm学习笔记（一）/vllm7.png)
...
![Worker 推理与 Sampler](/images/vllm学习笔记（一）/vllm8.png)
...
![请求完成与结果返回](/images/vllm学习笔记（一）/vllm9.png)
