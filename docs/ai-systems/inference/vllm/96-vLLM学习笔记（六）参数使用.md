...
sidebar_label: "96. vLLM 学习笔记（六）：参数使用"
sidebar_position: 96
description: "vLLM CLI 参数、默认值和支持状态变化很快。本文含 V0/V1 及不同发布版内容，只适合建立参数分类， 执行前必须以当前环境的 vllm serve --help=all 和固定版本官方文档为准。源码主线固定使用 vLLM v0.23.0。"
...
### 2.4 vLLM 与 vLLM-Ascend 区别 {/* #14-vllm-与-vllm-ascend-区别 */}
...
## 3. KV Cache 与内存 {/* #2-kv-cache-与内存 */}

与（一）中 **determine_num_available_blocks**、（四）**BlockSpaceManager** 的块数与显存分配对应。

### 3.1 block-size、gpu-memory-utilization {/* #21-block-sizegpu-memory-utilization */}
...
### 3.2 swap-space、CPU 卸载 {/* #22-swap-spacecpu-卸载 */}
...
### 4.1 max-num-seqs、max-num-batched-tokens {/* #31-max-num-seqsmax-num-batched-tokens */}
...
## 11. 参数取值说明（枚举/多选参数） {/* #10-参数取值说明枚举多选参数 */}

以下对**有固定可选值的参数**逐项说明每个取值的含义，便于按场景选择。

### 11.1 模型与加载 {/* #101-模型与加载 */}
...
### 11.2 KV Cache 与调度 {/* #102-kv-cache-与调度 */}
...
### 11.4 LoRA 与投机解码 {/* #104-lora-与投机解码 */}
...
### 11.6 布尔与数值类（常见含义） {/* #106-布尔与数值类常见含义 */}
...
### 13.1 先确认版本和实际参数 {/* #121-先确认版本和实际参数 */}
...
### 13.3 单机四卡 Tensor Parallel {/* #123-单机四卡-tensor-parallel */}
...
### 13.5 显式启用 Prefix Caching {/* #125-显式启用-prefix-caching */}
...
### 13.6 0.6.3 Swap 与当前 Offload 的版本差异 {/* #126-063-swap-与当前-offload-的版本差异 */}
...
### 13.9 OOM 的安全收敛顺序 {/* #129-oom-的安全收敛顺序 */}
