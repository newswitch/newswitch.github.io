...
### 4.1 策略 A：先完整 Prefill {/* #策略-a先完整-prefill */}
...
### 4.2 策略 B：Decode 永远优先，完整 Prefill 必须等 {/* #策略-bdecode-永远优先完整-prefill-必须等 */}
...
### 7.1 `max_num_batched_tokens` {/* #maxnumbatchedtokens */}
...
### 7.2 `max_num_seqs` {/* #maxnumseqs */}

决定一轮最多容纳的 Sequence 数；需要与 KV Cache 容量和典型上下文一起测。

### 7.3 `long_prefill_token_threshold` {/* #longprefilltokenthreshold */}

用于定义何种 Prompt 被视为长 Prefill。设置为 0 等行为随配置语义而定，应检查当前 CLI。

### 7.4 `max_num_partial_prefills` {/* #maxnumpartialprefills */}

限制同时存在多少 Partial Prefill。

### 7.5 `max_long_partial_prefills` {/* #maxlongpartialprefills */}

限制其中长 Prefill 数量。让该值小于总 Partial Prefill 数，可为短 Prompt 留出机会。

### 7.6 Scheduling Policy {/* #scheduling-policy */}
...
### 14.1 场景 A：Decode-only 倾向 {/* #场景-adecode-only-倾向 */}
...
### 14.2 场景 B：Prefill-only 倾向 {/* #场景-bprefill-only-倾向 */}
...
### 16.1 只提高 `max_num_batched_tokens` {/* #只提高-maxnumbatchedtokens */}

可能提高吞吐，但让 Decode ITL 尾部变差。

### 16.2 只提高 `max_num_seqs` {/* #只提高-maxnumseqs */}

可能先耗尽 KV Cache，产生更多抢占和排队。

### 16.3 只用平均长度压测 {/* #只用平均长度压测 */}
...
### 16.5 不区分 Prefix Cache 冷热 {/* #不区分-prefix-cache-冷热 */}

测试结果无法代表首次请求或真实多租户命中率。

### 16.6 把旧 V0 Swap 流程当成 V1 {/* #把旧-v0-swap-流程当成-v1 */}
