...
sidebar_position: 1
description: "单卡训练脚本能跑通后，上集群会立刻碰到：多进程怎么编号、谁当 Master、何时做集合通信、为什么「起了一半 Worker」会占死 GPU。本文给第六阶段搭地图——并行范式、进程角色、Rendezvous、以及在 Kubernetes / Volcano 上的任务生命周期。代码级 DDP 见……"
