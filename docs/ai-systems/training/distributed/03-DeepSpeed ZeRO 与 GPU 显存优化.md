...
DDP 让多卡「算得动」；当 **单卡装不下模型 + 优化器状态** 时，需要把状态切分到多卡——这就是 ZeRO / FSDP 一类方案。本文说明 ZeRO 1/2/3 在显存上的切什么、和 DDP 的关系，以及在 Kubernetes 上多留意的点。DDP 部署见 [第 30 篇](./02-PyTorch%20DDP%20在%20Kubernetes%20中的部署.md)。
...
3. 单机多卡通信便宜，优先把切分放同机；跨机 ZeRO-3 对网络极敏感（见 [NCCL](./05-NCCL%20通信原理与常见问题.md)）  
...
| Checkpoint | 分片权重需按框架 API 保存/加载（DeepSpeed / FSDP / DCP），见 [第 32 篇](./04-训练任务%20Checkpoint%20与断点恢复.md) |
...
- 团队只会 DDP、没有分片 CKPT 恢复预案：先把 [Checkpoint](./04-训练任务%20Checkpoint%20与断点恢复.md) 跑通再升阶段  
