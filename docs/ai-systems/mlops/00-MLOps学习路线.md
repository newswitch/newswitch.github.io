...
| 05 | [LLM 发布 Manifest 与制品完整性](./04-LLM发布Manifest与模型制品完整性.md) | 能把权重、Tokenizer、模板、镜像、参数和评测绑定为经过签名的发布单元 |
| 06 | [线上质量、漂移与反馈回流闭环](./05-线上质量漂移与反馈回流闭环.md) | 能从线上信号形成受控评测和改进数据，而不绕过隐私与发布门禁 |
