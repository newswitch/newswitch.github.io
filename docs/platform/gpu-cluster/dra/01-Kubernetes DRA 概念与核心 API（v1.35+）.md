...
本篇放在系列**最后学**：先掌握 [Device Plugin](../device-runtime/01-Kubernetes%20如何识别和管理%20GPU.md) 与整卡 `nvidia.com/gpu`，再理解 DRA 如何用 **Claim + 属性筛选** 做更细的设备分配。实践见 [第 62 篇](./02-DRA%20集群安装与设备分配实践（v1.34+）.md)。
