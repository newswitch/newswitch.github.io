...
可用 `tolerationSeconds: 300` 延迟驱逐。日常维护更推荐 `cordon` + `drain`（见 [第 12 篇](../device-runtime/08-GPU%20Operator%20升级、回滚与节点维护.md)），勿随意加 `NoExecute`。
...
下一篇可接共享模式对比：[整卡 / Time-Slicing / MPS / MIG](./07-GPU%20整卡独占、Time-Slicing、MPS%20与%20MIG%20对比.md)；配置细节见 [07 Pod 配置](../device-runtime/04-Kubernetes%20GPU%20Pod%20配置详解.md)。
