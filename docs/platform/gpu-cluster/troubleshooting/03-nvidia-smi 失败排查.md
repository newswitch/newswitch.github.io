...
> 示例以 NVIDIA 驱动、NVML、containerd/Toolkit 与 Kubernetes 为主。前置：[六层排障模型](./02-GPU%20集群六层排障模型.md)、[nvidia-smi 命令](../../../foundations/compute/gpu/06-nvidia-smi%20常用命令与指标说明.md)。
...
`cordon` → 迁业务 → 存 journal/`nvidia-smi -q`/`lspci -vvv` → Reset/重启/硬件诊断。见 [Xid 排查](./06-NVIDIA%20Xid%20错误排查.md)。
...
下一篇：[Pod 分配 GPU 后看不到 GPU](./04-Pod%20分配%20GPU%20后看不到%20GPU.md)。
