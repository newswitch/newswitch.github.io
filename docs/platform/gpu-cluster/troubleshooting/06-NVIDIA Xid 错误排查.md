...
| 79 | Fallen off bus | 隔离节点、存证、重启/硬件；见 [44](./03-nvidia-smi%20失败排查.md) |
...
告警：`DCGM_FI_DEV_XID_ERRORS`、ECC DBE、PCIe Replay 等；通知须含节点、GPU UUID、PCI、Xid、Pod、NS、时间——勿只发「GPU Xid 异常」。监控指标见 [第 38 篇](../../../engineering/observability/gpu/01-DCGM%20Exporter%20GPU%20监控指标详解.md)。
...
下一篇可对照 [NCCL Timeout](./07-NCCL%20Timeout%20排查流程.md)；节点整体异常见 [NotReady](./08-GPU%20节点%20NotReady%20的处理流程.md)。
