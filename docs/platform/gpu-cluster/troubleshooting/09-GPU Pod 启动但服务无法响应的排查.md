...
> 占位符（`NS`/`POD`/服务地址等）请换成自己的真实数据。前置：[探针](../../../ai-systems/inference/serving/04-大模型服务%20Kubernetes%20探针设计.md)、[六层排障](./02-GPU%20集群六层排障模型.md)。
...
下一篇排障后可接治理：[升级与变更管理](../governance/05-GPU%20集群升级与变更管理.md)。
