...
AI 负载还需要 GPU。在 [NVIDIA 驱动、CUDA 与容器运行时的关系](../../../foundations/compute/gpu/07-NVIDIA%20驱动、CUDA%20与容器运行时的关系.md) 里可以看到：Kubernetes 侧靠 **Device Plugin** 把节点上的 GPU 暴露成扩展资源，使用方式接近原生资源。
