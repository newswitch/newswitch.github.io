...
与本系列 [第 35 篇](../scheduling-sharing/12-GPU%20集群拓扑感知调度.md) 的 `nvidia-smi topo` 互补：一个管 **机内 NVLink**，一个管 **机架/区块** 放置。
