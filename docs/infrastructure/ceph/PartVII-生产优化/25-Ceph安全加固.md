...
### 误区一：CephX 已启用，所以流量已经加密

错误。CephX 提供认证和授权；流量机密性要看 msgr2 连接模式。

### 误区二：crc secure 表示强制 secure

错误。它是允许模式和优先顺序，通常先尝试 crc。

### 误区三：OSD 加密后，管理员也无法读取业务数据

错误。运行中的 OSD 已解锁，已授权客户端仍能读取。

### 误区四：把 keyring chmod 600 就完成密钥管理
...
至此，系列已经覆盖了 Ceph 从基础原理、部署、三类接口、监控、换盘、PG、容量、性能、网络、升级、灾备到安全治理的完整主线。后续可以进入真实事故复盘、自动化巡检、Kubernetes/Rook 和大型集群专项优化。

→ [第 23 篇：Ceph 自动化巡检与报告](./Ceph自动化巡检与报告)
