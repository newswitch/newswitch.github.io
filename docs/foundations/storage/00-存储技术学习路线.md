...
description: "从 Linux I/O 基础开始，系统学习本地存储、NFS、Ceph、对象存储、CSI、模型分发和 AI 工作负载数据路径。"
---
...
| 序号 | 文章 | 实验 |
| --- | --- | --- |
| 01 | 块、文件和对象存储 | 分别使用文件、块设备和 S3 API |
| 02 | Linux VFS 与一次 read 的完整路径 | 使用 strace、iostat、blktrace 观察 |
| 03 | 页缓存、预读与 Direct I/O | 比较冷读、热读和绕过缓存 |
| 04 | 存储性能指标与 fio 方法 | 设计顺序/随机、读/写、块大小实验 |
...
- NVMe 队列、Namespace 和性能模型。
- RAID、LVM 与文件系统选型。
- Local PV 调度、故障和数据恢复边界。
- Node-local 模型缓存回收与水位控制。
...
- [Ceph 三种接口在 AI 集群中的选型](./ceph/PartIX-AI场景/30-AI集群中的Ceph接口选型.md)
...
| 文章 | 技术重点 |
| --- | --- |
| S3 API 与一致性语义 | Bucket、Object、ETag、Versioning |
| Multipart 与 Range GET | 并行上传下载、断点续传 |
| 模型制品完整性 | Manifest、Checksum、签名和 provenance |
| 对象存储到节点缓存 | 并发控制、回源保护和缓存淘汰 |
...
- [Ceph 接入 Kubernetes](./ceph/PartIV-存储使用实战/15-Ceph接入Kubernetes.md)
