...
1. [`nvidia-smi`](../gpu/06-nvidia-smi%20常用命令与指标说明.md)：设备清单、利用率、显存、进程、拓扑、ECC 与 Xid 入口。
...
11. [`nsys`](../../../engineering/performance/03-Nsight-Systems端到端时间线分析.md)：先回答时间花在哪里、CPU 与 GPU 是否并行。
12. [`ncu`](../../../engineering/performance/04-Nsight-Compute-CUDA-Kernel分析.md)：再回答单个 Kernel 为什么慢。
