...
本文以 **NVIDIA GPU** 和 **CUDA** 体系为主，介绍计算结构、线程执行模型和存储层次，为后续 [nvidia-smi](./06-nvidia-smi%20常用命令与指标说明.md)、[驱动与 CUDA](./07-NVIDIA%20驱动、CUDA%20与容器运行时的关系.md)、Device Plugin、GPU Operator 和大模型部署打下基础。
...
更细的命令与指标见：[nvidia-smi 常用命令与指标说明](./06-nvidia-smi%20常用命令与指标说明.md)。
...
关注：型号、驱动版本、CUDA 兼容版本、温度、功耗、显存、GPU Util、进程。更完整示例见：[nvidia-smi 常用命令与指标说明](./06-nvidia-smi%20常用命令与指标说明.md)。
...
可看 GPU–GPU（含 NVLink）、与 CPU NUMA、与网卡的距离。后续见：[GPU 服务器硬件拓扑与 NUMA](./03-GPU%20服务器硬件拓扑与%20NUMA.md)。
...
显存原理补充：[HBM 显存原理：容量、带宽与访问效率](./02-HBM显存原理：容量、带宽与访问效率.md)；下一篇：[GPU 服务器硬件拓扑与 NUMA](./03-GPU%20服务器硬件拓扑与%20NUMA.md)；动手命令优先：[nvidia-smi 常用命令与指标说明](./06-nvidia-smi%20常用命令与指标说明.md)；组件链路：[NVIDIA 驱动、CUDA 与容器运行时的关系](./07-NVIDIA%20驱动、CUDA%20与容器运行时的关系.md)。
