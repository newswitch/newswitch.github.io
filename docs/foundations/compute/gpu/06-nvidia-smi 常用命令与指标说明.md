...
前置：[GPU 基础知识](./01-GPU%20基础知识：从计算核心到显存.md)、[硬件拓扑与 NUMA](./03-GPU%20服务器硬件拓扑与%20NUMA.md)。
...
驱动 / Toolkit 关系见：[NVIDIA 驱动、CUDA 与容器运行时的关系](./07-NVIDIA%20驱动、CUDA%20与容器运行时的关系.md)。
...
解读见：[GPU 服务器硬件拓扑与 NUMA](./03-GPU%20服务器硬件拓扑与%20NUMA.md)。
...
`nvidia-smi` 是日常 GPU 运维入口；写脚本时优先 `--query-gpu` + CSV，并记住 **Memory Util ≠ 显存容量占用**。下一篇：[NVIDIA 驱动、CUDA 与容器运行时的关系](./07-NVIDIA%20驱动、CUDA%20与容器运行时的关系.md)。
