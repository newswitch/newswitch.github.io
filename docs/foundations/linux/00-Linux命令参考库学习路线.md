...
CPU、内存与负载内容已经进入下一节；`taskset/chrt` 已在第 13 节的 CPU 拓扑与调度模块完成，systemd 服务生命周期已经进入第 11 节。跨模块的[Kubernetes 与容器命令参考库](../../platform/kubernetes/commands/00-Kubernetes与容器命令参考库学习路线.md)、[网络命令参考库](../networking/commands/00-网络命令参考库学习路线.md)、[存储命令参考库](../storage/commands/00-存储命令参考库学习路线.md)、[大数据学习地图](../../data-systems/00-大数据技术学习地图.md)和[GPU 与加速器命令参考库](../compute/commands/00-GPU与加速器命令参考库学习路线.md)继续直接复用。
