...
sidebar_label: "24. 24 · NCCL与HCCL多卡多机"
sidebar_position: 24
description: "系列：《NVIDIA＋昇腾双资源池 AI 推理集群：从 0 到部署、运维与故障排查》 阶段：第六阶段——两套机器部署推理 本文定位：并行通信原理、多机部署前验收与集合通信排障篇"
...
对照：[NCCL Timeout 排查](../../gpu/cluster/troubleshooting/07-NCCL%20Timeout%20排查流程.md) · [GPU 拓扑感知调度](../../gpu/cluster/scheduling/12-GPU%20集群拓扑感知调度.md)。
...
## 3. 理解 Rank、World Size 与进程组 {/* #三理解-rankworld-size-与进程组 */}
...
## 4. 四～五、NVIDIA 的 NCCL 路径与昇腾的 HCCL 路径 {/* #四五nvidia-的-nccl-路径与昇腾的-hccl-路径 */}
...
## 7. 八～九、控制面与数据面；跨机基础网络验收 {/* #八九控制面与数据面跨机基础网络验收 */}
...
## 8. 十～十一、NCCL 跨机验收与诊断变量 {/* #十十一nccl-跨机验收与诊断变量 */}
...
## 9. 十二～十三、HCCL 跨机验收与接口变量 {/* #十二十三hccl-跨机验收与接口变量 */}
...
## 10. Kubernetes 中的多机推理不能只靠普通 Deployment {/* #十四kubernetes-中的多机推理不能只靠普通-deployment */}
...
## 12. 同一个实例为什么不能混用 GPU 和 NPU {/* #十七同一个实例为什么不能混用-gpu-和-npu */}
...
## 14. 十九～二十、典型故障矩阵；不要用增大 Timeout 掩盖根因 {/* #十九二十典型故障矩阵不要用增大-timeout-掩盖根因 */}
...
## 15. 二十一～二十二、性能调优顺序与安全 {/* #二十一二十二性能调优顺序与安全 */}
...
## 16. 二十三～二十四、监控、告警与变更规则 {/* #二十三二十四监控告警与变更规则 */}
...
## 17. 二十五～二十七、生产验收、故障演练与练习 {/* #二十五二十七生产验收故障演练与练习 */}
