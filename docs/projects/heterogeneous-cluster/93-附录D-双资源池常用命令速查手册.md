...
sidebar_label: "93. 附录D · 命令速查"
sidebar_position: 93
description: "所属系列：《NVIDIA＋昇腾双资源池 AI 推理集群：从 0 到部署、运维与故障排查》 用途：安装验收、日常巡检和故障排查 约定： 表示必须替换的占位符，不能原样执行"
...
## 2. Linux 主机 {/* #二linux-主机 */}

### 2.1 系统与时间 · 只读 {/* #系统与时间--只读 */}
...
### 2.2 CPU、NUMA 与内存 · 只读 {/* #cpunuma-与内存--只读 */}
...
## 4. containerd 与 CRI {/* #四containerd-与-cri */}

### 4.1 版本和状态 · 只读 {/* #版本和状态--只读 */}
...
### 4.4 Runtime 配置 · 只读 {/* #runtime-配置--只读 */}
...
## 5. Kubernetes 集群 {/* #五kubernetes-集群 */}

### 5.1 上下文与版本 · 只读 {/* #上下文与版本--只读 */}
...
### 5.6 Service 和 Endpoint · 只读 {/* #service-和-endpoint--只读 */}
...
### 5.7 调度、配额、存储、HPA · 只读 {/* #调度配额存储hpa--只读 */}
...
### 6.2 修改 Label / Taint · 生产变更 {/* #修改-label--taint--生产变更 */}
...
## 7. NVIDIA 设备 {/* #七nvidia-设备 */}

### 7.1 设备与稳定标识 · 只读 {/* #设备与稳定标识--只读 */}
...
### 7.3 Device Plugin / DCGM · 只读 {/* #device-plugin--dcgm--只读 */}
...
### 8.2 Device Plugin / Exporter · 只读 {/* #device-plugin--exporter--只读 */}
...
## 11. Prometheus 与告警 {/* #十一prometheus-与告警 */}
...
## 12. NFS、Ceph 与模型文件 {/* #十二nfsceph-与模型文件 */}
...
- [专栏目录](./00-专栏目录.md)
- [附录 C：资产盘点模板](./92-附录C-服务器资产清单模板.md)
- [附录 E：双资源池 Kubernetes YAML 模板库](./94-附录E-双资源池Kubernetes-YAML模板库.md)
- [第 29 篇：NVIDIA 池专项运维](./29-NVIDIA资源池日常运维与故障排查.md)
- [第 30 篇：昇腾池专项运维](./30-昇腾资源池日常运维与故障排查.md)

← [附录 C](./92-附录C-服务器资产清单模板.md) · → [附录 E：YAML 模板库](./94-附录E-双资源池Kubernetes-YAML模板库.md)
