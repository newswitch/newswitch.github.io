...
sidebar_label: "96. 附录G · PromQL与看板"
sidebar_position: 96
description: "所属系列：《NVIDIA＋昇腾双资源池 AI 推理集群：从 0 到部署、运维与故障排查》 用途：把 NVIDIA 池、昇腾池和公共服务放进同一套可观测体系 原则：先统一指标语义和标签，再写告警；先确认指标真实存在，再复制 PromQL"
tags: [Prometheus, PromQL, Alertmanager, Grafana, DCGM, NPU Exporter, 附录]
date: 2026-08-07 96:00:00
categories: 云原生
---

# 附录G：PromQL、告警规则与Grafana看板清单
...
## 3. 公共层 PromQL 速查 {/* #三公共层-promql-速查 */}
...
## 4. vLLM 与 vLLM-Ascend 指标 {/* #四vllm-与-vllm-ascend-指标 */}
...
## 7. 统一网关 Recording Rule {/* #七统一网关-recording-rule */}
...
## 9. Alertmanager 分组与抑制 {/* #九alertmanager-分组与抑制 */}
...
- [附录 H：故障排查矩阵与决策树](./97-附录H-巡检与故障处理SOP.md)

← [附录 F](./95-附录F-模型显存HBM设备副本和故障容量计算表.md) · → [附录 H：故障排查矩阵与决策树](./97-附录H-巡检与故障处理SOP.md)
