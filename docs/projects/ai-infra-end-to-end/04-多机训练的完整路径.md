...
sidebar_label: "04. 多机训练的完整数据与通信路径"
sidebar_position: 4
description: "多机训练在单机路径上增加了调度、Rendezvous、交换网络和远端 GPU。任何一个 rank、网卡、链路或存储客户端变慢，都可能拖慢整个训练任务。"
...
### 16.1 Rendezvous 超时 {/* #rendezvous-超时 */}
...
### 16.5 RoCE 偶发超时 {/* #roce-偶发超时 */}

检查交换机 PFC/ECN、网卡队列、丢包、拥塞、MTU 和 DSCP/Traffic Class，不能只增大 Timeout。

### 16.6 Checkpoint 时 NCCL Timeout {/* #checkpoint-时-nccl-timeout */}
...
1. 普通RDMA在主机内存与HCA之间DMA，GPU数据通常先经过D2H/H2D；GDR让HCA直接访问已注册的GPU显存，缩短数据路径并减少CPU与主机内存复制。
2. 单机只验证NVLink/PCIe/SHM等节点内路径，多机还依赖HCA、RoCE/IB、GID、PFC/ECN、路由、GPU-HCA拓扑和GDR软件栈，因此单机通过不能推出多机通过。
3. Checkpoint/数据读取会与AllReduce争用NIC、交换机队列和主机IO，造成集合通信长尾、训练Step抖动，严重时触发超时。需要QoS、分网、限速或错峰。
4. 映射表应为每节点Rank0～7列出GPU索引/BDF、NUMA和HCA端口；全局Rank0～7属于节点A，8～15属于节点B，并保持相同的本地GPU-HCA邻接关系。
5. `NET/Socket`日志表示走TCP Socket，`NET/IB`表示走IB verbs/RoCE；保持测试条件相同时，比较带宽、延迟和CPU占用，同时确认日志中的HCA/GDR状态，不能只看最终吞吐。
6. 一个Worker延迟进入AllReduce时，其余Rank都等待该Collective，表现为同步放大的Step长尾；应通过各Rank时间线找到最先变慢的前置计算/IO，而不是把所有Rank超时当网络故障。
7. 四层验收为：端口与链路状态；主机内存`ib_*_bw`；GPU内存GDR点对点；`nccl-tests`多节点；最后真实训练。每层记录拓扑、消息大小、带宽、P95、错误计数和通过阈值。
