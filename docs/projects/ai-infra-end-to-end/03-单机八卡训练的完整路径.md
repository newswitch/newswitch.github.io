...
sidebar_position: 3
description: "“一台服务器有 8 张 GPU”只描述数量，没有说明 GPU 怎样连接。8 卡 PCIe 服务器、NVLink Bridge 服务器和 NVSwitch 服务器的通信能力可能完全不同。"
...
### 10.2 Tensor Parallel {/* #tensor-parallel */}
...
1. 一进程一卡让每个进程拥有独立CUDA Context和明确Local Rank，简化设备绑定、故障定位，并让DDP在反向阶段按Bucket执行集合通信。
2. DDP主要在反向传播期间对梯度做AllReduce，并在初始化/状态同步时通信；TP在单层前向和反向的矩阵分片之间频繁执行AllReduce、AllGather或ReduceScatter。
3. NCCL可能使用PCIe P2P；若P2P不可用则经共享内存和主机内存中转，极端情况下还可能走Socket。实际路径以拓扑和NCCL日志为准。
4. 对1/2/4/8卡使用相同消息范围和环境运行`all_reduce_perf`，横轴为消息大小、纵轴为`algbw/busbw`与P95耗时；同时报告卡数扩展效率，不能只贴峰值。
5. `topo -m`给出静态GPU/NIC邻接；`NCCL_DEBUG_SUBSYS=GRAPH`给出NCCL实际构造的Ring/Tree和通道。两者应能对应，若不对应要检查容器可见设备、ACS/IOMMU和拓扑文件。
6. 任一Rank晚进入集合通信，其他Rank都会在同一Collective等待，导致整个Step时间接近最慢Rank；日志/时间线会看到快Rank提前进入而长时间没有完成。
7. 逐级增加Worker并记录全局samples/s、每卡GPU空闲、CPU、IO和Batch准备P95；吞吐不再增长或CPU/内存压力显著上升时即超过有效并发点。
