...
sidebar_label: "00. 内核、硬件拓扑与中断命令导读"
sidebar_position: 0
...
[`lscpu`](../05-cpu-memory-load-proc/03-lscpu命令详解.md) 的完整说明位于 CPU 模块。网卡队列、RSS/RPS/XPS 与 coalescing 结合[网络命令参考库](../../../networking/commands/00-网络命令参考库学习路线.md)的 `ethtool`、`ip` 和 `tc` 学习；NVMe 相关命令参见[存储命令参考库](../../../storage/commands/00-存储命令参考库学习路线.md)。
