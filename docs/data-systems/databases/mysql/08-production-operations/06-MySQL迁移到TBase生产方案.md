...
### 20.1 参考答案与验收标准 {/* #参考答案与验收标准 */}

1. **构造兼容性样本**：为 `tenant`、`orders` 和 `order_items` 分别加入自增主键、布尔字段、时间字段和 JSON 字段，并执行一次 `INSERT ... ON DUPLICATE KEY UPDATE`。迁移前应保存 `SHOW CREATE TABLE`、样例数据和 SQL 执行结果，作为改造基线。
2. **目标 DDL**：自增列应改为 TBase 目标版本支持的序列或 identity 方案；`TINYINT(1)` 显式改为布尔或小整数；时间字段统一时区语义；JSON 按目标兼容能力选择 `json/jsonb` 或文本；Upsert 改为目标数据库支持的冲突处理语法。每项改造都必须说明数据范围、默认值和应用兼容性。
3. **验证倾斜**：将大量订单集中到同一个 `tenant_id`，再比较各数据节点的行数、容量和执行耗时。若一个节点承担绝大多数数据或请求，说明该分布键不适合超级租户，需要复合分布键、租户拆分或热点隔离。
4. **全量与增量闭环**：在一致性快照位点执行全量导出，同时从该位点启动 CDC；全量导入后继续消费增量，直到源端与目标端延迟归零。验收包括表级行数、主键范围、聚合校验和及抽样业务查询。
5. **CDC 故障恢复**：停止消费者并记录最后确认位点，恢复后从已提交位点继续；重复事件必须通过主键、事务标识或幂等 Upsert 消除；唯一冲突要进入隔离队列并保留原事件，不能直接跳过。
6. **执行计划比较**：带 `tenant_id` 的查询应能够路由到较少的数据节点；缺少分布键的查询通常需要广播、重分布或全节点扫描。使用 `EXPLAIN (ANALYZE, BUFFERS)` 比较扫描节点数、数据重分布量和实际耗时。
7. **正式切换**：先停止写入并记录时间，等待 CDC 追平，再执行最终校验；冻结旧连接入口，切换连接串并完成读写冒烟。只有错误率、延迟和关键业务结果均正常，才解除停写窗口。
8. **回滚**：目标端产生新订单后，不能直接把连接切回旧 MySQL。应先停止目标端写入，将切换后增量按经过演练的反向同步或业务补偿方案写回 MySQL，完成一致性校验后再恢复旧入口。
9. **迁移报告**：至少记录快照位点、CDC 位点、各阶段耗时、校验结果、冲突明细、切换和回滚时间、监控截图、执行人及遗留问题。验收标准是另一名工程师仅凭报告能够复盘并重做迁移。
