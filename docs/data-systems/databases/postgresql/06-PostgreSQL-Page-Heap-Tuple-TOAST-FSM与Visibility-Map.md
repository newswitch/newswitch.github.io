...
## 5. 从页证据理解膨胀和 Index-only Scan {/* #从页证据理解膨胀和-index-only-scan */}

在实验库安装 `pageinspect`、`pgstattuple`，插入并反复更新一批行，比较 VACUUM 前后 heap tuple、dead tuple、FSM/VM 和执行计划：

```sql
CREATE EXTENSION IF NOT EXISTS pageinspect;
CREATE EXTENSION IF NOT EXISTS pgstattuple;
CREATE EXTENSION IF NOT EXISTS pg_visibility;
SELECT * FROM pgstattuple('public.heap_lab');
SELECT blkno, all_visible, all_frozen FROM pg_visibility_map('public.heap_lab') LIMIT 20;
EXPLAIN (ANALYZE, BUFFERS) SELECT id FROM heap_lab WHERE id BETWEEN 1 AND 100;
```

扩展函数可能读取底层页并需要高权限，只在受控环境使用。HOT update 依赖被更新列不在索引且页面有空间；TOAST 只处理可变长大值，不会让宽行没有代价。VM 的 all-visible 位让 index-only scan 少访问 heap，VACUUM/写入会改变它。

膨胀治理先找长事务、autovacuum 进度、更新模式和 fillfactor，再决定 VACUUM、repack 或重建。直接 `VACUUM FULL` 会持有强锁并需要额外磁盘，不是默认修复。
