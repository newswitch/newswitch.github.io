...
## 4. Lua 与 Functions {/* #lua-与-functions */}
...
## 6. 版本基线与可执行实验 {/* #版本基线与可执行实验 */}

本文以 Redis Open Source 8.x/RESP3 为当前基线，同时说明 RESP2 客户端行为。先执行 `HELLO 3`、`INFO server` 和客户端兼容测试；Redis 8 将 Search、JSON 等能力整合进统一发行版，但旧版/托管版可用命令仍可能不同。

```bash
redis-cli --latency-history
redis-cli --pipe < commands.resp
redis-cli MONITOR   # 只在隔离实验短时使用
```

分别测逐条命令与不同 batch 的 pipeline，记录吞吐、P99、客户端内存和服务端输出缓冲。Pipeline 不是事务；`MULTI/EXEC` 不回滚运行期错误；Lua/Function 会阻塞主执行路径直到完成。脚本上线前验证 key 声明、超时、幂等、Cluster 同 slot 与失败重试，禁止把大循环当成“原子优化”。
