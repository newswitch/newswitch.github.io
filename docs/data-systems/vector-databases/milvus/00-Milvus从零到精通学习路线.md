...
| M16 | [Snapshot、Milvus Backup 与一致性恢复专项](./16-Milvus-Snapshot-Backup与一致性恢复专项.md) | P1 | 元数据、对象和日志状态怎样形成可恢复闭包 |
...
完成 M12～M16。建立向量条数×维度×类型、索引系数、副本数、加载比例、查询并发、写入速率、Compaction 和对象存储带宽模型，并验证快照与独立备份的恢复边界。
