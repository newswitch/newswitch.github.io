...
实验必须固定 embedding 模型、模型版本、维度、归一化方式和距离度量。用一小份数据执行精确检索建立 ground truth，再比较 HNSW 的 Recall@K、P95 和索引大小；只看查询耗时无法判断结果质量。

```http
GET rag_docs/_search
{
  "knn": {"field":"embedding","query_vector":[0.1,0.2,0.3],"k":10,"num_candidates":100,"filter":{"term":{"tenant":"a"}}},
  "_source":["doc_id","chunk","tenant"]
}
```

逐步改变 `num_candidates`，画出 recall/latency 曲线。过滤条件必须进入向量查询并做租户越权测试；检索后还要验证 chunk、去重、rerank、引用和答案质量。模型升级不能直接复用旧向量，应新建字段/索引完成双写、回填、对比和切换。
