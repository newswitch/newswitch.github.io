...
← [GPU 基础知识：从计算核心到显存](../fundamentals/01-GPU基础知识：从计算核心到显存.md)
...
1. 显存占用回答“分配了多少 GiB”；带宽利用回答“单位时间内读写链路有多忙或传输了多少数据”。模型可以占满显存但完全空闲，也可以占用不高却持续打满带宽。
2. HBM 仍比寄存器、Shared Memory 和缓存慢得多且能耗更高。分层存储通过数据复用、合并访问和减少重复 HBM 事务降低延迟与带宽需求。
3. Coalesced Access 是同一 Warp 的线程访问连续、对齐的地址，使多个线程请求合并为尽可能少的内存事务。跨度大或无规则访问会产生更多事务并浪费总线字节。
4. 训练除权重外还需保存梯度、优化器状态、前向激活和通信缓冲；推理通常只需权重、KV Cache 与临时工作区，因此训练显存通常更大。
5. 最常见状态是权重和 KV Cache 已驻留但请求很少；也可能是请求卡在 CPU、调度或通信阶段。应结合队列长度、Token 吞吐、功耗和 Kernel 时间线确认。
6. 逐元素算子通常算术强度低，DRAM 吞吐/活跃度高而计算管线利用较低；矩阵乘法若尺寸和布局合适，Tensor Core/SM 指标更高且数据复用更强。应以 Nsight Compute 的实际计数验证，而不是只看 `nvidia-smi`。
7. 有效带宽可按 `读字节 + 写字节 / 时间` 计算，并与同条件下的可持续基线比较。它低于理论值通常来自协议开销、访问不合并、缓存行为、时钟/功耗限制、并发不足和测量同步开销。
