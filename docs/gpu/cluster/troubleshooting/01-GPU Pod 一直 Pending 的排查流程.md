...
sidebar_label: "01. GPU Pod 一直 Pending 的排查流程"
sidebar_position: 1
description: "Pod 处于 Pending，表示已被 API 接受，但一个或多个容器尚未创建并启动。可能是调度失败，也可能是镜像拉取、存储挂载、Sandbox 创建等后续阶段问题。先判断是否已分配到节点。"
...
前置：[GPU Pod 配置详解](../device-management/04-Kubernetes%20GPU%20Pod%20配置详解.md)、[Device Plugin](../device-management/01-Kubernetes%20如何识别和管理%20GPU.md)。
...
关注：NVML 初始化失败、驱动未加载、找不到 `libnvidia-ml.so`、设备发现失败、Runtime 配置错误。部署细节：[Device Plugin 部署与配置](../device-management/02-NVIDIA-Device-Plugin部署与配置.md)。
...
关注：队列资源、`minMember` / Gang、队列是否关闭、优先级与抢占。见：[Volcano GPU 调度器入门](../scheduling/04-Volcano%20GPU%20调度器入门.md)。
...
常见：镜像拉取失败、NVIDIA Runtime 未配置、`FailedCreatePodSandBox`、CNI / PVC / 设备挂载失败。链路：[Pod 如何使用上 GPU](../device-management/03-Pod如何使用上GPU：Device%20Plugin与Container%20Toolkit.md)。
...
下一篇：[NVIDIA GPU Operator 架构与组件说明](../device-management/05-NVIDIA%20GPU%20Operator%20架构与组件说明.md)。
