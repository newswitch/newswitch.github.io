...
sidebar_position: 2
description: "GPU 节点成本高。不隔离时，Nginx、日志、Java、批处理也可能占掉 CPU/内存，影响模型业务。Taint 让节点排斥不符合条件的 Pod；Toleration 表示 Pod 可以容忍某污点。"
...
专用节点应同时用 **Taint + 标签/Affinity**：既挡普通 Pod，又让专用负载只进指定池。标签见：[节点标签与调度策略](./01-Kubernetes%20GPU%20节点标签与调度策略.md)。
...
可用 `tolerationSeconds: 300` 延迟驱逐。日常维护更推荐 `cordon` + `drain`（见 [第 12 篇](../device-management/08-GPU%20Operator%20升级、回滚与节点维护.md)），勿随意加 `NoExecute`。
...
下一篇可接共享模式对比：[整卡 / Time-Slicing / MPS / MIG](../sharing/07-GPU%20整卡独占、Time-Slicing、MPS%20与%20MIG%20对比.md)；配置细节见 [07 Pod 配置](../device-management/04-Kubernetes%20GPU%20Pod%20配置详解.md)。
