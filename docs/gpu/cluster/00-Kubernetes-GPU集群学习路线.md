...
#### 01.《[GPU 基础知识：从计算核心到显存](../fundamentals/01-GPU基础知识：从计算核心到显存.md)》
...
#### 02.《[GPU 服务器硬件拓扑与 NUMA](../pcie-numa/04-GPU服务器硬件拓扑与NUMA.md)》
...
#### 03.《[nvidia-smi 常用命令与指标说明](../commands/01-nvidia-smi常用命令与指标说明.md)》
...
#### 04.《[NVIDIA 驱动、CUDA 与容器运行时的关系](../driver-runtime/01-NVIDIA驱动CUDA与容器运行时的关系.md)》
...
#### 09.《[NVIDIA GPU Operator 架构与组件说明](./device-management/05-NVIDIA%20GPU%20Operator%20架构与组件说明.md)》
...
#### 10.《[使用 Helm 部署 GPU Operator](./device-management/06-使用%20Helm%20部署%20GPU%20Operator.md)》
...
#### 11.《[GPU Operator 两种驱动管理模式](./device-management/07-GPU%20Operator%20两种驱动管理模式.md)》

- Operator 管理驱动 vs 宿主机预装
- 适用场景与升级风险
- 生产环境建议

#### 12.《[GPU Operator 升级、回滚与节点维护](./device-management/08-GPU%20Operator%20升级、回滚与节点维护.md)》
...
#### 13.《[Kubernetes GPU 节点标签与调度策略](./scheduling/01-Kubernetes%20GPU%20节点标签与调度策略.md)》

- GFD 标签
- 型号 / 驱动 / MIG 能力选择
- 节点池与生产测试隔离

#### 14.《[GPU 节点 Taint 与 Toleration 实践](./scheduling/02-GPU%20节点%20Taint%20与%20Toleration%20实践.md)》

- 为什么隔离 GPU 节点
- Taint / Toleration 配置
- 防止普通 Pod 误入 GPU 节点

#### 15.《[GPU 集群优先级与抢占策略](./scheduling/03-GPU%20集群优先级与抢占策略.md)》

- PriorityClass
- 推理 / 训练 / 开发优先级分层
- 与 Volcano / Kueue 抢占的分工

#### 16.《[Volcano GPU 调度器入门](./scheduling/04-Volcano%20GPU%20调度器入门.md)》
...
#### 17.《[Volcano Queue 与 GPU 配额管理](./scheduling/05-Volcano%20Queue%20与%20GPU%20配额管理.md)》
...
#### 18.《[Gang Scheduling 在分布式训练中的作用](./scheduling/06-Gang%20Scheduling%20在分布式训练中的作用.md)》
...
#### 19.《[GPU 整卡独占、Time-Slicing、MPS 与 MIG 对比](./sharing/07-GPU%20整卡独占、Time-Slicing、MPS%20与%20MIG%20对比.md)》

- 隔离能力对比表
- 适用场景与选型建议

#### 20.《[Kubernetes GPU Time-Slicing 配置实践](./sharing/08-Kubernetes%20GPU%20Time-Slicing%20配置实践.md)》

- replicas 配置
- 共享验证与显存竞争实验
- 风险说明

#### 21.《[MIG 原理与 Kubernetes 配置](./sharing/09-MIG%20原理与%20Kubernetes%20配置.md)》
...
#### 34.《[AI 集群网络从零到精通](../../networking/ai-fabric/00-AI集群网络从零到精通学习路线.md)》

- Ethernet / IB / RoCE / RDMA / GPUDirect
- `NCCL_SOCKET_IFNAME`、IB/RoCE 检查
- `all_reduce_perf` 与 `ib_write_bw` 基线

#### 35.《[GPU 集群拓扑感知调度](./scheduling/12-GPU%20集群拓扑感知调度.md)》

- `nvidia-smi topo -m` / P2P / ACS
- NUMA / PCIe / NVLink 与装箱策略
- 单机多卡 vs 跨机调度决策

#### 36.《[大模型文件在 Kubernetes 中的存储方案](../../storage/ai-workloads/06-大模型文件在%20Kubernetes%20中的存储方案.md)》

- NFS / CephFS / 对象存储 / 本地 NVMe
- PVC / HostPath 风险
- 方案对比

#### 36b.《[AI 工作负载的存储 IO 模型](../../storage/ai-workloads/01-AI工作负载的存储IO模型.md)》

- 模型权重、训练数据与 Checkpoint 的访问模式
- 吞吐、IOPS、延迟、并发与元数据

#### 36c.《[GPUDirect Storage 原理与实践](../../storage/ai-workloads/02-GPUDirect-Storage原理与实践.md)》
...
#### 37.《[大模型冷启动优化](../../storage/ai-workloads/07-大模型冷启动优化.md)》
...
#### 38.《[DCGM Exporter GPU 监控指标详解](../../sre/observability/gpu/01-DCGM%20Exporter%20GPU%20监控指标详解.md)》

- 部署（Operator / Helm / Docker）与 Pod 映射
- 利用率 / 显存 / 温度 / 功耗 / 时钟
- Xid / ECC / PCIe / NVLink / MIG

#### 39.《[Prometheus GPU 告警策略设计](../../sre/observability/gpu/02-Prometheus%20GPU%20告警策略设计.md)》

- 高温、掉卡、Xid、ECC
- Device Plugin 异常、Pending、利用率异常
- 分级与持续时间（for）

#### 40.《[Grafana GPU 集群总览看板设计](../../sre/observability/gpu/03-Grafana%20GPU%20集群总览看板设计.md)》

- 四套看板：集群 / 节点卡 / Namespace·Pod·模型 / 推理业务

#### 41.《[GPU 利用率低但显存占满怎么分析](../../sre/observability/gpu/04-GPU%20利用率低但显存占满怎么分析.md)》

- 权重占用、请求不足、数据加载、NCCL 等待
- 排查指标与优化方向

#### 42.《[大模型业务指标与 GPU 指标关联分析](../../sre/observability/gpu/05-大模型业务指标与%20GPU%20指标关联分析.md)》
...
#### 57.《[生产级 Kubernetes GPU 集群架构设计](../../projects/production-gpu-cluster/01-生产级%20Kubernetes%20GPU%20集群架构设计.md)》
...
#### 58.《[GPU 集群完整部署实录](../../projects/production-gpu-cluster/02-GPU%20集群完整部署实录.md)》（示例实验模板）

- K8s + containerd + GPU Operator + Volcano
- Prometheus / Grafana / Alertmanager
- vLLM + 训练任务

#### 59.《[GPU 集群故障演练记录](../../projects/production-gpu-cluster/03-GPU%20集群故障演练记录.md)》（示例实验模板）

- 故障注入 → 告警 → 排查 → 恢复 → 改进

#### 60.《[Kubernetes GPU 集群学习总结](../../projects/production-gpu-cluster/04-Kubernetes%20GPU%20集群学习总结.md)》
