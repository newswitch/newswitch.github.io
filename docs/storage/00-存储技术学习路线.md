...
- 命令参考按“块设备与挂载 → 文件系统与容量 → 性能与介质 → LVM/RAID → NFS/RPC → Ceph → S3”组织。
- 每篇都包含对象模型、版本和参数族、输出判读、安全边界、生产排障与实验；危险的修复、格式化、阵列、对象删除和同步操作单独标记。

## 2. 第一阶段：存储与 Linux I/O 基础 {/* #二第一阶段存储与-linux-io-基础 */}
...
## 3. 第二阶段：本地存储与 NVMe {/* #三第二阶段本地存储与-nvme */}
...
- [高性能存储学习路线](./high-performance/00-高性能存储学习路线.md)
- [SPDK 从零到生产学习路线](./high-performance/spdk/00-SPDK从零到生产学习路线.md)
...
## 6. 第五阶段：对象存储与模型仓库 {/* #六第五阶段对象存储与模型仓库 */}
...
- [MinIO 从零到精通学习路线](./object-storage/minio/00-MinIO从零到精通学习路线.md)
...
| 文章 | 技术重点 |
| --- | --- |
| [MinIO 从零到精通学习路线](./object-storage/minio/00-MinIO从零到精通学习路线.md) | S3 数据路径、纠删码、Quorum、Healing、部署与生产治理 |
...
## 7. 第六阶段：Kubernetes CSI {/* #七第六阶段kubernetes-csi */}
...
## 8. 第七阶段：AI 工作负载 I/O {/* #八第七阶段ai-工作负载-io */}
...
进入[AI 训练数据面学习路线](./training-data-plane/00-AI训练数据面从零到生产学习路线.md)，系统掌握 Dataset Shard、DataLoader、Lustre/BeeGFS、JuiceFS/Alluxio/Fluid、本地 NVMe 缓存和 Checkpoint Storm。该模块负责回答“存储吞吐看起来正常，为什么所有 GPU 仍在等数据”。
...
- [ ] 能用 Data Wait、Cache Hit、元数据和 H2D 指标定位训练数据瓶颈。
