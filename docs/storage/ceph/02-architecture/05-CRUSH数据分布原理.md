...
## 1. 为什么 Ceph 不使用中心位置表 {/* #为什么-ceph-不使用中心位置表 */}
...
## 3. CRUSH Map：描述真实物理拓扑 {/* #crush-map描述真实物理拓扑 */}
...
## 4. Bucket：CRUSH 层级中的容器 {/* #bucketcrush-层级中的容器 */}
...
## 5. Failure Domain：副本需要跨越什么故障边界 {/* #failure-domain副本需要跨越什么故障边界 */}

Failure Domain 就是故障域，用于指定数据副本应该分散到哪一级物理边界。

### 5.1 OSD 级故障域 {/* #1-osd-级故障域 */}
...
### 5.4 台服务器应该选什么故障域 {/* #4-10-台服务器应该选什么故障域 */}
...
## 6. CRUSH Rule：数据放置规则 {/* #crush-rule数据放置规则 */}
...
### 6.1 查看 CRUSH Rule {/* #查看-crush-rule */}
...
### 6.2 Pool 与 CRUSH Rule 的关系 {/* #pool-与-crush-rule-的关系 */}
...
## 7. Device Class：让不同介质服务不同业务 {/* #device-class让不同介质服务不同业务 */}
...
### 7.1 Device Class 不是自动性能保证 {/* #device-class-不是自动性能保证 */}
...
## 8. Weight：容量越大，承担的数据通常越多 {/* #weight容量越大承担的数据通常越多 */}

CRUSH 需要知道不同设备的相对容量，才能让大容量 OSD 承担更多数据，小容量 OSD 承担更少数据。

### 8.1 CRUSH Weight {/* #1-crush-weight */}
...
### 8.2 OSD Reweight {/* #2-osd-reweight */}
...
## 9. CRUSH 如何选择三副本位置 {/* #crush-如何选择三副本位置 */}
...
## 10. 扩容时为什么不是所有数据都重写 {/* #扩容时为什么不是所有数据都重写 */}
...
### 10.1 扩容为什么会影响业务性能 {/* #扩容为什么会影响业务性能 */}
...
## 11. CRUSH 与 Up Set、Acting Set {/* #crush-与-up-setacting-set */}
