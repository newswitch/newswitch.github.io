...
1. RBD Image被切分为一系列RADOS对象，Image元数据记录大小、对象布局、特性、快照和父子关系；客户端把块偏移映射为对象并直接访问OSD。
2. krbd是Linux内核RBD驱动，映射为标准块设备，兼容文件系统/传统工具；librbd是用户态库，QEMU、容器存储等可直接调用并更快获得新特性。选择取决于功能、内核和性能。
3. ext4/XFS默认不是共享磁盘集群文件系统，两台主机同时写会各自缓存并修改同一元数据，造成损坏。RBD Exclusive Lock只解决部分协调，不会把普通文件系统变成多主机安全文件系统。
4. `rbd pool init`为Pool设置RBD应用标记并初始化RBD需要的基础对象/元数据，使工具和管理功能把该Pool正确识别为RBD Pool。
5. Image逻辑大小可以远大于实际已分配对象，所有稀疏Image的承诺容量可能超过Raw可用空间；后续同时写满会触发full，因此必须监控实际使用、最大承诺和增长率。
6. RBD扩容只增大块设备边界，分区表和文件系统仍保持原大小；还需刷新设备、扩分区/LVM并使用对应文件系统在线/离线扩容工具。
7. Crash-consistent相当于突然断电时的磁盘状态，可能需要日志恢复；Application-consistent会冻结/flush应用和文件系统，保证事务边界与内存状态按应用要求落盘。
8. Clone最初通过Copy-on-Write引用父快照的未修改对象，父快照是数据依赖且通常受保护；删除父快照前需删除子Clone或Flatten解除依赖。
9. Flatten把仍引用父快照的数据复制到Clone自身，增加容量和后台IO，过程中可能影响性能；完成后Clone独立，父快照才可安全清理。
10. Trash先隐藏并保留Image一段时间，给误删提供可恢复窗口，还可设置延迟删除；直接`rm`通常难以撤销。Trash仍占容量，必须配合到期清理和监控。
