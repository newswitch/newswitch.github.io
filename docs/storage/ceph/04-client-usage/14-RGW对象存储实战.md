...
## 2. 对象存储不是「另一个共享目录」 {/* #对象存储不是另一个共享目录 */}
...
## 3. RGW 在 Ceph 中处于什么位置 {/* #rgw-在-ceph-中处于什么位置 */}
...
### 3.1 RGW 进程坏了会丢数据吗 {/* #rgw-进程坏了会丢数据吗 */}
...
## 5. 使用 cephadm 部署 RGW {/* #使用-cephadm-部署-rgw */}

### 5.1 给 RGW 主机添加标签 {/* #给-rgw-主机添加标签 */}
...
## 9. 使用 AWS CLI 访问 Ceph RGW {/* #使用-aws-cli-访问-ceph-rgw */}

RGW 提供 S3 兼容 API，因此可以使用 AWS CLI、SDK、s3cmd、mc 等工具。

### 9.1 配置独立 Profile {/* #配置独立-profile */}
...
### 9.5 删除测试对象和 Bucket {/* #删除测试对象和-bucket */}
...
## 10. 用户、Bucket、ACL 和 Policy 的关系 {/* #用户bucketacl-和-policy-的关系 */}
...
### 11.1 配额不是容量规划的替代品 {/* #配额不是容量规划的替代品 */}
...
### 11.2 统计可能不是实时精确值 {/* #统计可能不是实时精确值 */}

RGW 为了性能会缓存和异步维护部分统计信息。因此容量页面与瞬时写入量可能存在短暂差异。做审计时可同步统计，但不要高频执行重统计操作给生产集群增加额外负担。

## 12. 版本控制、生命周期与误删除保护 {/* #版本控制生命周期与误删除保护 */}

### 12.1 开启 Bucket 版本控制 {/* #开启-bucket-版本控制 */}
...
### 14.1 数据池和索引池的负载不同 {/* #数据池和索引池的负载不同 */}
...
### 14.2 绝对不要绕过 RGW 删除对象 {/* #绝对不要绕过-rgw-删除对象 */}
...
## 15. 单集群高可用与 Multisite {/* #单集群高可用与-multisite */}

### 15.1 单集群多 RGW {/* #单集群多-rgw */}
...
### 15.2 RGW Multisite {/* #rgw-multisite */}
...
### 17.3 检查 Endpoint、区域和签名方式 {/* #检查-endpoint区域和签名方式 */}
...
## 19. 常见故障三：返回 5xx 或请求超时 {/* #常见故障三返回-5xx-或请求超时 */}

### 19.1 先区分入口问题和存储问题 {/* #先区分入口问题和存储问题 */}
...
## 20. 常见故障四：上传或下载很慢 {/* #常见故障四上传或下载很慢 */}

### 20.1 先建立可重复的测试 {/* #先建立可重复的测试 */}
...
## 21. Bucket 索引热点与大 Bucket {/* #bucket-索引热点与大-bucket */}
...
下一篇将进入 Kubernetes 场景，讲清 Rook/CSI、StorageClass、PVC，以及如何同时排查两套控制面。

→ [第 15 篇：Ceph 接入 Kubernetes](./15-Ceph接入Kubernetes.md)
