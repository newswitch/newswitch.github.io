...
## 4. 为什么 cephadm 能滚动升级 {/* #为什么-cephadm-能滚动升级 */}
...
## 12. 关于 PG autoscaler {/* #关于-pg-autoscaler */}
...
## 13. CephFS 的升级注意事项 {/* #cephfs-的升级注意事项 */}
...
### 15.2 Ceph 状态进度 {/* #ceph-状态进度 */}

```bash
watch -n 10 ceph -s
```

### 15.3 cephadm 事件日志 {/* #cephadm-事件日志 */}
...
## 18. UPGRADE_NO_STANDBY_MGR {/* #upgradenostandbymgr */}
...
## 19. UPGRADE_FAILED_PULL {/* #upgradefailedpull */}
...
## 22. DAEMON_OLD_VERSION {/* #daemonoldversion */}
...
## 24. OSD 升级期间的特殊原则 {/* #osd-升级期间的特殊原则 */}
...
## 30. 一份可直接采用的升级 Runbook {/* #一份可直接采用的升级-runbook */}
...
### 33.1 误区一：HEALTH_OK 就可以升级 {/* #误区一healthok-就可以升级 */}

还要验证容量、主机、镜像、客户端、备份和业务基线。

### 33.2 误区二：容器化就能随便回滚镜像 {/* #误区二容器化就能随便回滚镜像 */}

Ceph 是有状态系统，协议和元数据兼容性比容器标签更重要。

### 33.3 误区三：upgrade stop 会恢复旧版本 {/* #误区三upgrade-stop-会恢复旧版本 */}

它只停止后续编排。

### 33.4 误区四：一次升级同时做内核、网络和 CRUSH 变更更省窗口 {/* #误区四一次升级同时做内核网络和-crush-变更更省窗口 */}

这样很难归因，也会叠加故障风险。

### 33.5 误区五：升级期间告警全部可以忽略 {/* #误区五升级期间告警全部可以忽略 */}

临时 `HEALTH_WARN` 可能正常，但每条告警必须可解释。

### 33.6 误区六：守护进程全是新版本就算完成 {/* #误区六守护进程全是新版本就算完成 */}
...
1. Ceph只保证文档中声明的升级路径；跨越不支持的中间版本可能跳过数据格式、配置迁移和兼容检查，导致Daemon无法加入或无法回退。必须逐版阅读Release Notes。
2. MGR承载cephadm升级编排和模块，先升级可让控制逻辑理解新版本；MON先于数据Daemon升级可建立兼容的集群Map/协议基础。之后再按编排顺序滚动OSD等服务。
3. Active MGR升级/重启时需Standby接管，避免Dashboard、指标、编排和管理模块长时间不可用；升级前应确认Standby与模块状态正常。
4. `pause`保留升级状态但暂不继续；`resume`从暂停点继续；`stop`结束当前升级任务并清除继续推进意图。具体版本行为要用`ceph orch upgrade status`确认。
5. 已经升级的Daemon和可能完成的数据/配置变更不会因`stop`自动降级，它只是停止继续滚动；回退需要单独受支持的方案，很多大版本并不支持直接降级。
6. 检查镜像引用/架构与Digest、Registry DNS/路由/TLS/认证/限流、代理、主机时间、容器运行时存储、每台Host到Registry连通性及私有CA分发，保留`cephadm`与runtime pull错误。
7. 升级会逐个停止Daemon并可能触发短时降级/恢复，镜像和日志也占本地空间；容量过高时任一OSD不可用都可能没有恢复目的地并触发full。
8. 多Active MDS包含Rank、Standby-Replay、客户端能力和元数据负载迁移，版本兼容与Failover行为更复杂；需按版本说明评估缩Rank、客户端兼容和专门验收。
9. 验证客户端RBD读写/快照、CephFS文件语义与MDS、RGW S3、CSI动态创建/挂载、备份/镜像、监控告警、性能P99和错误日志，而不仅是Daemon版本一致。
10. 同时改多层会扩大故障域且无法归因，回滚路径互相依赖。应一次只改变一类变量，在每个阶段建立健康和业务验收点。
