...
## 6. CRUSH 层级必须对应真实世界 {/* #crush-层级必须对应真实世界 */}
...
## 7. CRUSH 标签质量比层级数量更重要 {/* #crush-标签质量比层级数量更重要 */}
...
## 8. failure domain 不等于地理容灾 {/* #failure-domain-不等于地理容灾 */}
...
## 12. 看 PG replica，不只看 PG 总数 {/* #看-pg-replica不只看-pg-总数 */}
...
## 13. 优先使用 PG autoscaler {/* #优先使用-pg-autoscaler */}
...
## 14. mon_target_pg_per_osd 不要照抄旧经验 {/* #montargetpgperosd-不要照抄旧经验 */}
...
## 15. bulk、target size 和边界 {/* #bulktarget-size-和边界 */}
...
## 27. BlueStore DB/WAL {/* #bluestore-dbwal */}
...
## 30. mClock：业务与后台任务的 QoS {/* #mclock业务与后台任务的-qos */}
...
## 31. 恢复策略应按时段和风险切换 {/* #恢复策略应按时段和风险切换 */}
...
## 33. Balancer 与数据分布 {/* #balancer-与数据分布 */}
...
## 34. 容量扩容线不能等于 nearfull {/* #容量扩容线不能等于-nearfull */}
...
### 47.1 误区一：容量越大，集群就越难 {/* #误区一容量越大集群就越难 */}

对象、PG、Pool、客户端和恢复窗口可能比 TB/PB 更关键。

### 47.2 误区二：一个集群可以无限横向扩展 {/* #误区二一个集群可以无限横向扩展 */}

数据面可扩展不代表控制面和运维爆炸半径没有边界。

### 47.3 误区三：PG 越多分布越好，所以越多越好 {/* #误区三pg-越多分布越好所以越多越好 */}

过多 PG 会增加内存、peering 和控制面开销。

### 47.4 误区四：加大 recovery 并发一定缩短事故 {/* #误区四加大-recovery-并发一定缩短事故 */}

磁盘/网络已饱和时，更多并发可能恶化业务和有效吞吐。

### 47.5 误区五：更多 MON 会更可靠、更快 {/* #误区五更多-mon-会更可靠更快 */}

MON 依赖多数派，3/5 个常见；无限增加会增加一致性成本。

### 47.6 误区六：扩容就是一次加入全部新盘 {/* #误区六扩容就是一次加入全部新盘 */}
...
掌握大规模治理后，可以继续深入源码级原理、协议抓包、BlueStore/RocksDB 专项和生产案例复盘，并把分析结果反馈到容量模型、变更策略与 Runbook。
...
1. “大规模”还取决于OSD/PG/Pool数量、对象数、变化频率、租户、故障域和团队认知负荷；大量小盘、小对象或频繁变更的容量小集群同样复杂。
2. 单大集群容量池化和管理效率高，但故障/升级爆炸半径大、控制面和恢复压力集中；多集群隔离和独立演进好，但容量碎片、成本、监控与运维重复增加。
3. CRUSH依赖标签判断Failure Domain；若多个Host错误标成同Rack或同一物理Rack被标成多个逻辑Rack，三副本可能实际落在同一故障域，Rack故障同时丢多副本。
4. OSD实际承载的是PG副本/EC分片。总PG数只算Primary逻辑PG，不反映`size`或`k+m`后的Daemon负担；应看每OSD的PG replica及分布。
5. `bulk`告诉autoscaler这是预期承载大量数据的Pool，可更早分配合理PG；`target_size_bytes/ratio`提供未来容量预期，帮助在尚未写满前按目标分配PG。
6. 每个Pool带来PG、配置、监控和autoscaler负担，租户数很大时会让控制面和运维复杂度爆炸；多数隔离应优先用Namespace、CephX、Subvolume或Bucket。
7. 小对象增加BlueStore/RocksDB元数据、索引和空间开销，Scrub要遍历更多条目，恢复也要处理更多对象与消息，导致IOPS和CPU而非带宽成为瓶颈。
8. 它是BlueStore缓存调节目标，不限制进程其他内存，也存在调整延迟和峰值；线程栈、库、元数据及碎片会让RSS超过目标，硬边界应由节点/cgroup容量管理并留余量。
9. 单Host OSD越多，该Host故障丢失的数据和PG副本越多，需要在剩余网络/OSD上重建；若要求在固定小时内恢复，OSD密度必须受剩余聚合带宽、目的空间和前台SLO约束。
10. 分批能观察CRUSH迁移量、恢复P99和硬件一致性，并在错误拓扑时停止；与升级叠加会同时改变容量布局和软件行为，难以归因且回滚相互影响。
