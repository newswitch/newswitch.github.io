...
## 2. 为什么 Ceph 对网络特别敏感 {/* #为什么-ceph-对网络特别敏感 */}
...
## 4. Public Network 与 Cluster Network {/* #public-network-与-cluster-network */}

### 4.1 Public Network {/* #public-network */}
...
### 4.2 Cluster Network {/* #cluster-network */}
...
## 14. 场景一：OSD 反复 down/up {/* #场景一osd-反复-downup */}
...
## 15. 场景二：MON quorum 不稳定 {/* #场景二mon-quorum-不稳定 */}
...
### 19.1 Ping 不丢包，所以网络没问题 {/* #ping-不丢包所以网络没问题 */}

错误。小 ICMP 包不能覆盖 TCP、MTU、拥塞、带宽、交换机缓冲和业务并发。

### 19.2 双网一定比单网快 {/* #双网一定比单网快 */}

错误。低速双网可能不如高速冗余单网，而且复杂度更高。

### 19.3 两条链路聚合后单流速度自动翻倍 {/* #两条链路聚合后单流速度自动翻倍 */}

错误。单流通常落在一个成员接口。

### 19.4 Jumbo Frame 一定提升性能 {/* #jumbo-frame-一定提升性能 */}

错误。收益取决于 CPU、包率和工作负载，配置不一致反而制造隐蔽故障。

### 19.5 OSD heartbeat timeout 一定是网卡坏了 {/* #osd-heartbeat-timeout-一定是网卡坏了 */}

错误。CPU stall、磁盘阻塞、进程卡顿也可能让心跳超时。

### 19.6 临时关防火墙最快 {/* #临时关防火墙最快 */}
...
## 21. 一份生产网络排障 Runbook {/* #一份生产网络排障-runbook */}

### 21.1 阶段 A：定义影响 {/* #阶段-a定义影响 */}
...
1. 客户端把写入交给Primary后，Primary还要把数据或EC分片发送给其他OSD并等待确认；三副本会产生副本网络和磁盘写放大，恢复时还会额外复制。
2. Public Network承载客户端到MON/OSD以及Daemon公共通信；Cluster Network主要承载OSD复制、Heartbeat、Recovery和Backfill。没有Cluster Network时全部共享Public。
3. 中小集群、单网络已具备足够带宽和冗余、团队难以维护双平面时，单网络更简单可靠；双网络只有在能真正隔离故障/容量且配置一致时才有价值。
4. LACP/ECMP通常按五元组哈希把单个TCP流固定到一个成员口，增加的是多流聚合与冗余，不保证单流跨两口条带化。
5. IPv4 ICMP头8字节、IPv4头20字节，Payload 8972加28正好9000字节；`-M do`禁止分片，可验证端到端9000 MTU。若有额外隧道封装还要扣除其开销。
6. 优先怀疑MTU/PMTU黑洞、分片被阻止或中间设备MTU不一致；同时检查大包ping、TCP重传、接口丢弃和封装开销。
7. Heartbeat线程也可能因OSD磁盘长时间阻塞、CPU饥饿、内存回收、进程暂停或主机卡顿而不能及时发送/处理，网络只是其中一层。
8. 清空规则会扩大攻击面、破坏其他业务并丢失现场。应保存规则和计数器，只对精确源/目的/端口增加临时受控规则或使用抓包/conntrack验证。
9. 完全停止会让PG长期degraded，暴露于第二次故障并延长恢复窗口。应先保护前台SLO，再按可接受带宽/延迟调节Recovery参数，而不是无限期停掉。
10. 提供精确时间窗、源/目的IP与端口、方向、VLAN/VRF、节点/Rack、MTU、路径、重传/丢包计数、接口错误、MTR/抓包、正常与异常对照、业务影响及已排除项，避免只写“网络慢”。
