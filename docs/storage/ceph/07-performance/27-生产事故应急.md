...
sidebar_label: "27. Ceph 生产事故应急与复盘：接警、止损、证据、恢复和防止复发"
sidebar_position: 27
...
### 4.1 Incident Commander {/* #incident-commander */}
...
### 4.2 Ceph Operator {/* #ceph-operator */}
...
### 4.4 Business/Client Owner {/* #businessclient-owner */}

- 验证业务
- 决定是否停写或降级
- 提供错误率、延迟和受影响范围

### 4.5 Infrastructure Support {/* #infrastructure-support */}
...
## 11. 绝对不要在信息不足时做的事 {/* #绝对不要在信息不足时做的事 */}
...
## 12. 场景一：单个 OSD down {/* #场景一单个-osd-down */}
...
## 13. 场景二：整台 OSD 主机失联 {/* #场景二整台-osd-主机失联 */}
...
## 14. 场景三：多个 OSD 同时 down {/* #场景三多个-osd-同时-down */}
...
## 15. 场景四：MON quorum 丢失 {/* #场景四mon-quorum-丢失 */}
...
## 16. 场景五：集群 full 或接近 full {/* #场景五集群-full-或接近-full */}
...
## 17. 场景六：PG inactive、incomplete 或 stale {/* #场景六pg-inactiveincomplete-或-stale */}
...
## 18. 场景七：PG inconsistent / scrub error {/* #场景七pg-inconsistent--scrub-error */}
...
## 19. 场景八：大量 slow ops {/* #场景八大量-slow-ops */}
...
## 20. 场景九：OSD flapping {/* #场景九osd-flapping */}
...
## 23. 场景十二：CephFS 业务异常 {/* #场景十二cephfs-业务异常 */}
...
## 25. 场景十四：cephadm 无法管理主机 {/* #场景十四cephadm-无法管理主机 */}
...
## 27. 场景十六：误操作或自动化失控 {/* #场景十六误操作或自动化失控 */}
...
1. 止损控制影响继续扩大；恢复让服务回到可接受状态；永久修复消除已知缺陷；根因分析解释事件为何发生及为何未被阻止。四者时间点和验收条件不同。
2. 批量重启会破坏现场、扩大Quorum/副本损失，并把“慢”变成“不可用”。前五分钟应冻结高风险变更、确认影响面、保存时间线并找到第一处异常。
3. 立即out会触发整机数据量的Backfill；若主机很快恢复会制造双重迁移。应先判断电源/网络共同故障、预计恢复时间、容量余量和第二故障风险。
4. 保护拥有合法多数派Quorum、最新一致状态且能安全Fence另一侧的一方；不能为了“节点更多”强行合并两个分区。先恢复通信/隔离少数侧，避免双写假设。
5. 它明确接受无法找到对象的永久丢失，可能让业务数据不可恢复且不可逆，必须在备份、影响对象、业务Owner和替代方案全部确认后最高级审批。
6. `inconsistent`不等于可删除，可能只有某副本损坏且仍有正确副本。应先列出差异、检查介质并确定权威副本，repair或对象级处置都需证据。
7. 当继续写会加剧数据不一致/容量full、灾备切换需Fencing、依赖服务已失去正确性、写错误高且无法幂等重放，或止损收益大于可用性损失时应停写。
8. 预期改善什么；最坏副作用和故障域是什么；如何验证成功、何时停止并回滚。执行人还应记录命令、对象和时间。
9. 根因是没有它事故不会发生的基础缺陷；触发因素是使缺陷在该时刻暴露的事件；放大因素让影响更大或恢复更慢，例如缺告警、容量不足和未演练备份。
10. 改为“新增OSD Raw使用率与预测到full时间告警：70% Warning、80% Critical，持续10分钟；在测试环境注入指标验证5分钟内送达OnCall；Owner为存储组，截止日期和Dashboard/Runbook链接明确”。
