...
### 3.5 同一集群提供多种存储接口 {/* #5-同一集群提供多种存储接口 */}
...
## 4. 台 2TB 服务器到底能用多少空间 {/* #10-台-2tb-服务器到底能用多少空间 */}
...
## 5. Ceph 不是什么 {/* #ceph-不是什么 */}

理解 Ceph 的能力，也要理解它的边界。

### 5.1 Ceph 不等于备份 {/* #1-ceph-不等于备份 */}
...
### 5.2 Ceph 不一定比单机存储延迟更低 {/* #2-ceph-不一定比单机存储延迟更低 */}

Ceph 的数据需要通过网络访问，并且可能涉及多副本写入。它的优势是分布式能力、扩展性和可靠性，不是所有场景下的单次 IO 延迟都一定低于本地 NVMe。

### 5.3 Ceph 不是装完就不用维护 {/* #3-ceph-不是装完就不用维护 */}
...
### 5.4 Ceph 并不适合所有小规模场景 {/* #4-ceph-并不适合所有小规模场景 */}
...
## 6. NFS 与 Ceph 怎么选择 {/* #nfs-与-ceph-怎么选择 */}
...
1. 客户端只是共同访问同一台NFS Server上的同一个2TB文件系统，挂载数量不会复制或叠加服务端磁盘容量，因此总共享容量仍是2TB。
2. OSD管理磁盘上的对象数据，处理客户端/其他OSD读写、复制或EC分片、恢复、回填、Scrub并上报状态，是Ceph数据面的核心进程。
3. 三副本理论业务容量为`20 TB / 3 ≈ 6.67 TB`，尚未扣除BlueStore、元数据、预留空间和故障恢复余量。
4. Ceph需要空闲空间完成数据迁移、恢复和写放大；接近full阈值时部分OSD/Pool会被保护性限写，100%还会让恢复陷入无空间可放的死局。
5. 多副本处于同一管理域，会同步逻辑删除、误操作和数据损坏，也可能同时遭遇集群级灾难；备份必须具备独立故障域、历史版本和经过验证的恢复路径。
