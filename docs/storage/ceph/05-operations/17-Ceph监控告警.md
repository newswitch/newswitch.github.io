...
不要从未经版本验证的资料复制 PromQL 后直接上线告警。至少确认：
...
建立生产监控后，应继续用 OSD 故障与磁盘更换、PG 异常、容量危机、网络故障、性能回退和升级维护等场景验证告警是否真正可行动。
...
1. `HEALTH_OK`只说明Ceph已知的集群健康检查没有异常，不能证明客户端网络、CSI、权限、网关、应用IO、延迟和数据语义均正常。
2. Prometheus采集并存储时序指标；Grafana查询展示Dashboard；Alertmanager对告警分组、路由、抑制、静默并发送通知。三者职责互补。
3. 客户端、操作系统、磁盘SMART、交换网络、容器/CSI和业务SLO可能先出问题，而Ceph控制面尚未告警；需要端到端指标和探测。
4. `inactive`表示PG不能正常服务IO，直接影响可用性，应最高优先；`degraded`通常仍active可访问，但冗余下降，需要尽快恢复并防止第二次故障。
5. 单盘故障与整机/交换机/电源故障的处置完全不同。先按Host/Rack聚合down OSD，避免把共同故障误判为多块盘同时损坏并逐盘破坏性更换。
6. full ratio只是保护阈值，调高不会新增空间，还缩小恢复缓冲并可能把OSD推到真正写满。应扩容、清理或迁移数据。
7. 可能来自客户端阻塞、网络丢包、OSD磁盘/DB慢、CPU/内存压力、锁竞争、PG peering、恢复/Backfill、介质错误和整机抖动，需要沿操作阶段定位。
8. 至少包含触发条件、影响判断、只读诊断命令、证据保存、分级处置、停止条件、升级联系人、恢复验证和回滚/风险说明。
9. 分组减少同一事故通知风暴；抑制隐藏已被上游根因解释的下游告警；静默必须自动到期，避免维护结束后永久失去告警保护。
10. 集群指标恢复不代表客户端和数据已恢复；业务冒烟、一致性检查和SLO确认用于关闭事故，复盘则把触发/放大因素转化为可验证改进，防止复发。
