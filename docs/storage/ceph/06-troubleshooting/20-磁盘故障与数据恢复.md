...
## 3. 先理解 OSD 的四种组合状态 {/* #先理解-osd-的四种组合状态 */}
...
### 3.1 为什么 OSD down 后不会立刻 out {/* #为什么-osd-down-后不会立刻-out */}
...
### 3.2 out 不等于磁盘已经可以拔 {/* #out-不等于磁盘已经可以拔 */}

out 只表示不再作为正常数据放置目标。必须等待 PG 从该 OSD 迁移完成，并通过安全检查，才能进行销毁或物理更换。

## 4. ok-to-stop 与 safe-to-destroy 的区别 {/* #ok-to-stop-与-safe-to-destroy-的区别 */}

这两个检查经常被混淆。

### 4.1 ok-to-stop {/* #ok-to-stop */}
...
### 4.2 safe-to-destroy {/* #safe-to-destroy */}
...
## 5. 收到 OSD down 告警后的第一轮检查 {/* #收到-osd-down-告警后的第一轮检查 */}
...
### 5.1 同一主机多个 OSD 同时 down {/* #同一主机多个-osd-同时-down */}
...
### 5.2 只有单个 OSD down {/* #只有单个-osd-down */}
...
## 6. 准确定位 OSD 对应的设备 {/* #准确定位-osd-对应的设备 */}

### 6.1 从编排器查看 OSD 所在主机 {/* #从编排器查看-osd-所在主机 */}
...
### 6.3 在 OSD 主机查看 ceph-volume 映射 {/* #在-osd-主机查看-ceph-volume-映射 */}
...
### 6.4 `/dev/sdX` 不是可靠身份 {/* #devsdx-不是可靠身份 */}
...
## 7. 判断是进程问题还是硬件问题 {/* #判断是进程问题还是硬件问题 */}

### 7.1 检查 OSD 日志 {/* #检查-osd-日志 */}
...
### 7.3 检查 SMART 或 NVMe 健康 {/* #检查-smart-或-nvme-健康 */}
...
### 8.1 场景 A：OSD 进程异常，设备健康 {/* #场景-aosd-进程异常设备健康 */}
...
### 8.3 场景 C：磁盘确认永久故障 {/* #场景-c磁盘确认永久故障 */}
...
## 10. cephadm 推荐换盘流程 {/* #cephadm-推荐换盘流程 */}

### 10.1 调度 OSD 替换 {/* #调度-osd-替换 */}
...
### 10.5 为什么不要随便使用 `--force` {/* #为什么不要随便使用---force */}
...
### 11.2 热插拔不代表可以无流程拔盘 {/* #热插拔不代表可以无流程拔盘 */}

硬件支持热插拔只说明接口层允许在线操作，不代表 Ceph 数据已经安全迁走。必须同时满足 Ceph 安全条件和硬件操作要求。

### 11.3 共享 DB/WAL 设备要特别小心 {/* #共享-dbwal-设备要特别小心 */}
...
## 12. 让 cephadm 识别新盘 {/* #让-cephadm-识别新盘 */}
...
### 12.1 新盘不是 AVAILABLE {/* #新盘不是-available */}
...
## 13. 重新部署 OSD {/* #重新部署-osd */}

### 13.1 使用已有 OSDSpec {/* #使用已有-osdspec */}
...
## 14. 换盘后的恢复验证 {/* #换盘后的恢复验证 */}

### 14.1 验证 OSD 状态 {/* #验证-osd-状态 */}
...
## 15. 计划性换盘与故障换盘的差别 {/* #计划性换盘与故障换盘的差别 */}
...
### 16.1 OSD 移除一直 draining {/* #osd-移除一直-draining */}
...
### 16.2 新盘没有自动创建 OSD {/* #新盘没有自动创建-osd */}
...
### 16.3 新 OSD ID 没有复用 {/* #新-osd-id-没有复用 */}
...
### 16.4 换盘后 PG 一直 degraded {/* #换盘后-pg-一直-degraded */}
...
1. `down+in`表示OSD不可达但仍是CRUSH期望成员，PG可能等待其短时恢复；`down+out`表示不可达且已退出正常放置，Ceph会把数据重建到其他OSD。
2. `ok-to-stop`判断当前停止某Daemon/Host是否会立即破坏可用性或冗余约束；`safe-to-destroy`判断某OSD的数据是否已有安全副本、能否永久销毁。后者不是前者的同义词。
3. `/dev/sdc`会因重启、插拔和控制器枚举变化而漂移。应同时核对OSD ID、FSID、序列号、WWN、`ceph-volume lvm list`、槽位灯和SMART，避免拔错盘。
4. 同Host全部down更像主机电源、网络、容器运行时、共享DB设备或背板故障。逐盘替换会破坏本来完好的数据并制造大规模恢复，应先按共同故障域诊断。
5. ceph-volume/cephadm的替换流程可在新设备上重建Daemon并保留原OSD ID，从而保留运维身份和部分映射语义；仍需验证目标版本命令和数据恢复完成。
6. `noout`长期存在会让永久丢失的OSD仍被视为期望位置，PG持续degraded且无法充分重建冗余；确认永久故障后应按流程out/replace并解除临时Flag。
7. 持久DriveGroup或`all-available-devices`规格仍匹配该设备；zap清除签名后它重新成为Available，协调器会自动创建OSD。先修改规格再zap。
8. 一个DB/WAL设备可能承载多个数据OSD的元数据，故障会同时使多OSD不可用；更换需重建映射、评估数据盘是否可复用，并按多个OSD共同故障计算恢复容量。
9. `up`只表示新Daemon在线。还需`in`、CRUSH位置/Device Class正确、PG恢复完成、无slow ops、SMART正常、容量均衡和业务SLO通过。
10. 出现PG inactive/unfound、MON Quorum风险、第二块盘/整机异常、目的OSD接近full、恢复网络/磁盘过载、设备身份不一致、`safe-to-destroy`不通过或业务P99越过停止线时，必须暂停。
