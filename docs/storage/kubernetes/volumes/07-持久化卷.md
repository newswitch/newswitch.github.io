...
sidebar_label: "07. 持久化卷（Persistent Volume）"
sidebar_position: 7
...
### 2.5 回收阶段（Reclaiming） {/* #回收阶段reclaiming */}
...
## 3. PersistentVolume 配置详解 {/* #persistentvolume-配置详解 */}
...
#### 3.2.1 存储容量（Capacity） {/* #存储容量capacity */}

定义 PV 的存储容量，使用标准 Kubernetes 资源单位。目前主要支持存储大小，未来可能扩展支持 IOPS、吞吐量等属性。

#### 3.2.2 访问模式（Access Modes） {/* #访问模式access-modes */}
...
#### 3.2.3 卷模式（Volume Mode） {/* #卷模式volume-mode */}

- **Filesystem**：以文件系统方式挂载（默认）
- **Block**：以原始块设备方式使用，适合高性能场景

#### 3.2.4 节点亲和性（Node Affinity） {/* #节点亲和性node-affinity */}
...
## 4. PersistentVolumeClaim 配置详解 {/* #persistentvolumeclaim-配置详解 */}
...
## 6. StorageClass 配置 {/* #storageclass-配置 */}

StorageClass 支持多种参数和策略，适配不同存储后端和业务需求。

### 6.1 基础 StorageClass 示例 {/* #基础-storageclass-示例 */}
