...
sidebar_label: "02. ConfigMap 和 Secret 管理"
sidebar_position: 2
...
![ConfigMap 与 Secret 消费方式](/images/k8s/storage/configuration-management/264b709cf94d9fb0c15e96c22c1adcf7.svg)
...
## 2. ConfigMap 详解 {/* #configmap-详解 */}

ConfigMap 以键值对形式存储配置信息，实现配置与应用代码解耦，提升应用可移植性。

### 2.1 ConfigMap 结构 {/* #configmap-结构 */}
...
### 2.2 ConfigMap 消费方式 {/* #configmap-消费方式 */}
...
![ConfigMap 消费方式](/images/k8s/storage/configuration-management/2c1fa159a5e8f941ddbc7886ecee0593.svg)

- 作为环境变量注入
- 作为所有环境变量批量注入
- 以卷文件挂载到容器

## 3. Secret 详解 {/* #secret-详解 */}

Secret 用于存储敏感信息，如密码、OAuth 令牌、SSH 密钥等。与 ConfigMap 类似，但有额外的安全考量。

### 3.1 Secret 类型 {/* #secret-类型 */}
...
![Secret 类型与典型用途](/images/k8s/storage/configuration-management/38bb71880763e102d2958db70a8762de.svg)
...
### 3.3 Secret 在 Pod 中的使用 {/* #secret-在-pod-中的使用 */}
...
## 4. Secret 安全性注意事项 {/* #secret-安全性注意事项 */}
...
![Secret 安全风险与缓解措施](/images/k8s/storage/configuration-management/92ee38bdc2df2f01cc9cb32d8123be1b.svg)
...
![ConfigMap/Secret 生命周期](/images/k8s/storage/configuration-management/a48ed307fe2bbba38a1011156957baf8.svg)

- 卷挂载方式：变更会最终同步到 Pod（不适用于 subPath 挂载），同步延迟取决于 kubelet 周期与缓存。
- 环境变量方式：变更不会自动同步，需重启 Pod。

## 6. 不可变 ConfigMap 与 Secret {/* #不可变-configmap-与-secret */}
...
## 7. ConfigMap 与 Secret 选择指南 {/* #configmap-与-secret-选择指南 */}
...
![ConfigMap vs Secret 选择流程](/images/k8s/storage/configuration-management/07d6e9b9d96fa077ded5e471b799b624.svg)
...
## 9. Kubernetes Secret 的替代方案 {/* #kubernetes-secret-的替代方案 */}
