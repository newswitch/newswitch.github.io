...
sidebar_label: "02. LeaderWorkerSet 插件"
sidebar_position: 2
description: "大规模语言模型（LLM）推理采用多节点、多 GPU 的分布式计算架构，通过 Tensor Parallelism 跨 GPU 切分模型参数，结合 Pipeline Parallelism 跨节点分配计算阶段，实现多设备协同推理。传统的 Kubernetes Deployment 和 State……"
...
## 5. 使用示例：vLLM 分布式推理 {/* #5-使用示例vllm-分布式推理 */}
