...
## 1. 什么是 Operator SDK {/* #什么是-operator-sdk */}

[Operator SDK](./07-Operator-SDK.md) 是由 Red Hat 开源的用于构建 Kubernetes Operator 的开发框架。它提供了工具、库和标准化模式，帮助开发者高效、一致地创建、测试和打包能够自动化管理 Kubernetes 上复杂应用程序的 Operator。

Operator SDK 基于 Kubernetes controller-runtime 库构建，并通过与 Operator Lifecycle Manager (OLM) 的无缝集成，为 Operator 特定的工具和抽象提供了扩展支持。

## 2. 为什么需要 Operator SDK {/* #为什么需要-operator-sdk */}
...
## 4. 安装 Operator SDK {/* #安装-operator-sdk */}
...
#### 4.2.1 方式一：使用二进制文件（推荐） {/* #方式一使用二进制文件推荐 */}
...
#### 4.2.2 方式二：使用 Homebrew (macOS) {/* #方式二使用-homebrew-macos */}
...
## 7. 创建第一个 Operator 项目 {/* #创建第一个-operator-项目 */}
...
## 8. Operator 实现类型 {/* #operator-实现类型 */}

Operator SDK 支持三种主要实现方式，适配不同场景和技术栈。

### 8.1 Go-based Operators {/* #go-based-operators */}

基于 Go 的 Operator 直接使用 controller-runtime，适合复杂业务逻辑和高度自定义需求。

### 8.2 Ansible-based Operators {/* #ansible-based-operators */}

Ansible Operator 利用 Ansible playbook 和角色，无需 Go 编程，适合已有自动化脚本的场景。

### 8.3 Helm-based Operators {/* #helm-based-operators */}
...
## 9. Operator Lifecycle Manager 集成 {/* #operator-lifecycle-manager-集成 */}
...
## 12. 部署 Operator {/* #部署-operator */}

Operator 支持 OLM Bundle 部署和传统部署两种方式。

### 12.1 Bundle 管理和 OLM 部署 {/* #bundle-管理和-olm-部署 */}

#### 12.1.1 创建 Operator Bundle {/* #创建-operator-bundle */}
...
#### 12.2.1 安装 CRD {/* #安装-crd */}

```bash
make install
```

#### 12.2.2 本地运行 (开发) {/* #本地运行-开发 */}
