...
sidebar_label: "01. 扩展 Kubernetes 概览"
sidebar_position: 1
...
## 3. 为什么需要扩展 Kubernetes {/* #为什么需要扩展-kubernetes */}
...
1. [API 扩展机制](./02-API扩展机制.md) —— 了解如何定义新的 Kubernetes 资源类型
2. [API 聚合层（APIService）](./03-APIService.md) —— 理解聚合层工作原理与历史背景
3. [自定义资源定义（CRD）](./04-CRD.md) —— 学习主流扩展方式与 OpenAPI 校验机制
4. [控制器与 Operator 模式](./05-控制器与Operator模式.md) —— 掌握自定义控制循环的实现
5. [Kubebuilder 实战](./06-Kubebuilder.md) —— 从零构建控制器
6. [Operator SDK 实战](./07-Operator-SDK.md) —— 企业级 Operator 开发框架
7. [准入控制扩展](./08-Admission-Webhook扩展.md) —— 掌握动态策略与安全校验
8. [ValidatingWebhook](./09-Validating-Webhook扩展.md) 与 [MutatingWebhook](./10-Mutating-Webhook扩展.md) —— 实战示例
9. [调度扩展机制](./11-调度架构扩展.md) —— 理解调度流程与 Framework
10. [Scheduler Framework 插件](./12-Scheduler-Framework插件.md) —— 自定义调度逻辑
11. [GPU 与 AI 调度](./14-GPU与AI调度.md) —— 面向 AI 原生场景的调度优化
