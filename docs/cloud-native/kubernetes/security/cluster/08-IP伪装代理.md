...
![IP 伪装代理工作原理示意图](/images/k8s/security/ip-masq-agent/ip-masq-1.webp)
...
## 4. 部署 IP 伪装代理 {/* #部署-ip-伪装代理 */}

ip-masq-agent 支持默认和自定义两种部署方式，适应不同集群环境需求。

### 4.1 使用默认配置部署 {/* #使用默认配置部署 */}
...
### 7.3 验证 iptables 规则 {/* #验证-iptables-规则 */}
