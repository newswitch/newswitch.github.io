...
sidebar_label: "11. Kubernetes 中的用户与身份认证授权"
sidebar_position: 11
...
## 1. 理解 Kubernetes 中的用户类型 {/* #理解-kubernetes-中的用户类型 */}
...
### 1.2 Service Account {/* #service-account */}
...
### 3.3 Bootstrap Token {/* #bootstrap-token */}
...
### 3.5 Service Account Token {/* #service-account-token */}
...
### 3.6 OpenID Connect Token {/* #openid-connect-token */}

[OpenID Connect](https://openid.net/connect/)（OIDC）基于 OAuth2，可集成外部身份提供商。

下图展示了 OIDC 认证流程：

![Kubernetes OpenID Connect Flow](/images/k8s/security/authentication/kubernetes-oidc-login.webp)
...
### 3.7 Webhook Token 认证 {/* #webhook-token-认证 */}
...
### 5.1 使用 OpenSSL 生成证书 {/* #使用-openssl-生成证书 */}
...
### 5.2 使用 certificates.k8s.io API {/* #使用-certificatesk8sio-api */}
...
- [使用 kubeconfig 或 token 进行用户身份认证](./10-kubeconfig和token认证.md)
