...
## 2. 使用 kubeconfig 文件认证 {/* #使用-kubeconfig-文件认证 */}

kubeconfig 文件是 Kubernetes 客户端（如 kubectl）与集群安全通信的核心配置文件，支持基于证书的身份认证。

### 2.1 kubeconfig 文件生成 {/* #kubeconfig-文件生成 */}

关于如何生成 kubeconfig 文件，请参考 [创建用户认证授权的 kubeconfig 文件](./09-Kubeconfig用户认证授权.md)。

### 2.2 Dashboard 认证的特殊要求 {/* #dashboard-认证的特殊要求 */}
...
## 3. Service Account Token 认证 {/* #service-account-token-认证 */}

Service Account token 认证适用于自动化脚本、第三方工具和 Dashboard 等场景，便于权限隔离和细粒度控制。

### 3.1 创建集群管理员 Token {/* #创建集群管理员-token */}
...
### 3.2 获取管理员 Token {/* #获取管理员-token */}

获取 ServiceAccount token 有多种方式，适配不同 Kubernetes 版本。

#### 3.2.1 方法一：kubectl describe（推荐） {/* #方法一kubectl-describe推荐 */}
...
#### 3.2.2 方法二：创建临时 Token（Kubernetes 1.24+） {/* #方法二创建临时-tokenkubernetes-124 */}
...
#### 3.2.3 方法三：手动创建 Secret（长期 token） {/* #方法三手动创建-secret长期-token */}
...
### 3.3 为特定命名空间创建用户 Token {/* #为特定命名空间创建用户-token */}
...
- `admin`：命名空间管理权限
- 更细粒度权限请参考 [RBAC——基于角色的访问控制](../identity/03-RBAC.md)
