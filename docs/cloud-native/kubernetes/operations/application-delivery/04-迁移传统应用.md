...
sidebar_label: "04. 迁移传统应用到 Kubernetes 步骤详解——以 Hadoop YARN 为例"
sidebar_position: 4
...
本文档主要介绍如何将已有的传统分布式应用程序迁移到 Kubernetes 中。如果你想要直接开发 Kubernetes 原生应用，可以参考 [适用于 Kubernetes 的应用开发部署流程](./03-应用开发部署流程.md)。
...
![将单体应用迁移到云原生 (图片来自 DevOpsDay Toronto)](/images/k8s/devops/migrating-hadoop-yarn-to-kubernetes/migrating-monolith-to-kubernetes.webp)

本文将以 Spark on YARN 迁移为例进行详细说明。该例子具有足够的复杂性和典型性，掌握这个案例有助于理解大部分传统应用的迁移过程。

下图是整个架构的示意图，所有进程管理和容器扩容通过 Makefile 实现：

![Spark on YARN with Kubernetes](/images/k8s/devops/migrating-hadoop-yarn-to-kubernetes/spark-on-yarn-with-kubernetes.webp)
...
![术语](/images/k8s/devops/migrating-hadoop-yarn-to-kubernetes/terms-in-kubernetes-app-deployment.webp)

为了更好地理解迁移细节，本文所有操作都通过命令手动完成，不使用自动化工具。待充分理解细节后，可以引入自动化工具优化流程，提高效率并减少人为错误。

## 3. 迁移实施步骤 {/* #迁移实施步骤 */}

![分解步骤解析](/images/k8s/devops/migrating-hadoop-yarn-to-kubernetes/migrating-hadoop-yarn-to-kubernetes.webp)

### 3.1 第一步：应用服务化拆解 {/* #第一步应用服务化拆解 */}
...
### 3.2 第二步：容器镜像制作 {/* #第二步容器镜像制作 */}

根据服务拆解结果，需要制作以下镜像：

#### 3.2.1 Hadoop 基础镜像 {/* #hadoop-基础镜像 */}
...
### 3.4 第四步：Kubernetes 资源定义 {/* #第四步kubernetes-资源定义 */}
...
#### 3.4.1 ResourceManager StatefulSet 示例 {/* #resourcemanager-statefulset-示例 */}
...
### 3.6 第六步：ConfigMap 创建 {/* #第六步configmap-创建 */}
