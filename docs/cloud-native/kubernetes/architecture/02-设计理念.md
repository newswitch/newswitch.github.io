...
sidebar_label: "02. Kubernetes 的设计理念"
sidebar_position: 2
...
![Kubernetes 分层架构](/images/k8s/architecture/perspective/ae715519606b429d7b341fbe4cedc29a.svg)
...
## 4. Kubernetes 的核心技术概念和 API 对象 {/* #kubernetes-的核心技术概念和-api-对象 */}
...
### 4.2 副本控制器（Replication Controller，RC） {/* #副本控制器replication-controllerrc */}
...
### 4.7 后台支撑服务集（DaemonSet） {/* #后台支撑服务集daemonset */}
...
### 4.8 有状态服务集（StatefulSet） {/* #有状态服务集statefulset */}
...
### 4.9 集群联邦（Federation） {/* #集群联邦federation */}
...
### 4.11 持久存储卷（Persistent Volume，PV）和持久存储卷声明（Persistent Volume Claim，PVC） {/* #持久存储卷persistent-volumepv和持久存储卷声明persistent-volume-claimpvc */}
...
### 4.14 用户帐户（User Account）和服务帐户（Service Account） {/* #用户帐户user-account和服务帐户service-account */}
