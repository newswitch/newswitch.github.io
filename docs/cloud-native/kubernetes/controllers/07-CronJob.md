...
CronJob 管理基于时间的 [Job](./06-Job.md)，即可以在给定时间点只运行一次，也可以周期性地在给定时间点运行。一个 CronJob 对象类似于 *crontab*（cron table）文件中的一行。它根据指定的预定计划周期性地运行一个 Job，格式可以参考 [Cron](https://en.wikipedia.org/wiki/Cron)。
...
- **`.spec.jobTemplate`**：Job 模板，指定需要运行的任务，格式同 [Job](./06-Job.md)
...
## 4. 创建 CronJob {/* #创建-cronjob */}

可以通过 YAML 文件或 kubectl 命令创建 CronJob 资源。

### 4.1 使用 YAML 文件 {/* #使用-yaml-文件 */}
...
### 4.2 使用 kubectl 命令 {/* #使用-kubectl-命令 */}
...
## 5. 管理 CronJob {/* #管理-cronjob */}

日常运维中，需关注 CronJob 的状态、相关 Job 和 Pod 的执行情况。

### 5.1 查看 CronJob 状态 {/* #查看-cronjob-状态 */}
...
### 5.2 查看相关 Job 和 Pod {/* #查看相关-job-和-pod */}
...
## 6. CronJob 限制和注意事项 {/* #cronjob-限制和注意事项 */}
