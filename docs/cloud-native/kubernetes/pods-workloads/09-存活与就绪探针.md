...
sidebar_label: "09. 配置 Pod 的 liveness 和 readiness 探针"
sidebar_position: 9
...
![Kubernetes 探针生命周期流程](/images/k8s/pod/liveness-readiness-probes/e35048912a19d0596bcb9df7532a1a7a.svg)

### 1.1 Liveness Probe（存活探针） {/* #liveness-probe存活探针 */}

Kubelet 使用 liveness probe 判断容器是否需要重启。当进程死锁或进入不可恢复状态时，liveness 探针可自动触发重启，提升系统自愈能力。

### 1.2 Readiness Probe（就绪探针） {/* #readiness-probe就绪探针 */}
...
### 2.1 基于命令的 Liveness 探针 {/* #基于命令的-liveness-探针 */}
...
### 2.2 基于 HTTP 的 Liveness 探针 {/* #基于-http-的-liveness-探针 */}
...
### 2.3 基于 TCP 的 Liveness/Readiness 探针 {/* #基于-tcp-的-livenessreadiness-探针 */}
...
## 3. Readiness 探针配置示例 {/* #readiness-探针配置示例 */}
...
![探针参数关系](/images/k8s/pod/liveness-readiness-probes/2823dc43fbec206e134991780b4452e0.svg)
...
## 6. 启动探针（Startup Probe）【2024 新推荐】 {/* #启动探针startup-probe2024-新推荐 */}
