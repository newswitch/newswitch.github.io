...
![Pod 结构模式](/images/k8s/pod/overview/ff2d948b185ecfb1cf4be1059c0859db.svg)
...
## 4. Pod 的生命周期管理 {/* #pod-的生命周期管理 */}

Pod 的生命周期管理是保障应用高可用和自动化运维的关键。

### 4.1 为什么不直接使用 Pod {/* #为什么不直接使用-pod */}
...
| 控制器类型 | 用途 | 特点 |
|------------|------|------|
| [Deployment](../controllers/02-Deployment.md) | 无状态应用 | 副本管理、滚动更新 |
| [StatefulSet](../controllers/03-StatefulSet.md) | 有状态应用 | 有序部署、持久化存储 |
| [DaemonSet](../controllers/04-DaemonSet.md) | 节点级服务 | 每个节点运行一个 Pod |
| [Job](../controllers/06-Job.md) | 批处理任务 | 一次性任务执行 |
| [CronJob](../controllers/07-CronJob.md) | 定时任务 | 按计划执行任务 |
