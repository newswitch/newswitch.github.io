...
![OAM 规范示意图](/images/k8s/cloud-native/spec/oam-spec.webp)
...
## 3. Workload - 工作负载定义 {/* #workload---工作负载定义 */}
...
### 3.1 Workload 定义示例 {/* #workload-定义示例 */}
...
### 4.2 Component 组成部分 {/* #component-组成部分 */}

`Component` 定义由以下几个部分组成：

#### 4.2.1 metadata {/* #metadata */}

关于 Component 的信息，主要是针对应用运维的信息。

#### 4.2.2 workload {/* #workload */}
...
## 6. ApplicationScope - 应用范围 {/* #applicationscope---应用范围 */}

`ApplicationScope` 根据 `Component` 中的应用逻辑或共同行为划定作用域，将其分组以便于管理。

### 6.1 ApplicationScope 特征 {/* #applicationscope-特征 */}
...
### 6.2 支持的 ApplicationScope 类型 {/* #支持的-applicationscope-类型 */}
...
### 6.3 NetworkScope 示例 {/* #networkscope-示例 */}
...
## 7. ApplicationConfiguration - 应用配置 {/* #applicationconfiguration---应用配置 */}
...
### 7.1 ApplicationConfiguration 示例 {/* #applicationconfiguration-示例 */}
...
### 8.1 应用开发者（Application Developer） {/* #应用开发者application-developer */}

- 定义 `Component`，描述应用的功能逻辑
- 专注于业务代码实现，无需关心运维细节
- 可以定义 `parameters` 来允许运维人员调整配置

### 8.2 应用运维人员（Application Operator） {/* #应用运维人员application-operator */}

- 创建 `ApplicationConfiguration`，组合 `Component` 和 `Trait`
- 配置运维特性如扩缩容、路由、监控等
- 管理应用的部署、升级和监控

### 8.3 基础设施运维人员（Infrastructure Operator） {/* #基础设施运维人员infrastructure-operator */}
...
### 9.1 Component 设计 {/* #component-设计 */}

- 保持 Component 的单一职责，一个 Component 只做一件事
- 合理设计 parameters，让运维人员能够灵活配置
- 使用语义化的命名和详细的描述

### 9.2 Trait 应用 {/* #trait-应用 */}

- 优先使用平台提供的标准 Trait
- 避免在同一个 Component 上应用冲突的 Trait
- 按照依赖关系合理排序 Trait

### 9.3 ApplicationScope 规划 {/* #applicationscope-规划 */}
